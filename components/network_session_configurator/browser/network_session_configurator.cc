...
bool AreQuicParamsValid(const base::CommandLine& command_line,
                        ...
                        const VariationParameters& quic_trial_params) {
  // The --force-fieldtrial-params escape hatch used to short-circuit this
  // check. components/variations is gone, so no caller can force params and
  // every set of params reaching here is validated for real.
  if (!base::EqualsCaseInsensitiveASCII(
