...
void TestGpuServiceHolder::InitializeOnGpuThread(
  ...
  if (gpu_preferences.gr_context_type == gpu::GrContextType::kGraphiteDawn) {
    // This used to create a DawnContextProvider and require it succeed.
    // Dawn is gone, so there's nothing to create; leave
    // init_params.dawn_context_provider unset and let GpuServiceImpl's
    // (already Dawn-less) initialization handle the fallback.
