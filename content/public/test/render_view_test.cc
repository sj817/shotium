...
gfx::Rect RenderViewTest::GetElementBounds(const std::string& element_id) {
  // This used to run a JavaScript snippet that walked offsetParent chains.
  // There is no JavaScript engine, so ask the DOM directly: BoundsInWidget()
  // is the element's border box in widget coordinates, which is what the old
  // script computed (offset chain + offsetWidth/offsetHeight).
  blink::WebElement element =
      GetMainFrame()->GetDocument().GetElementById(
          blink::WebString::FromUtf8(element_id));
  if (element.IsNull()) {
    return gfx::Rect();
  }
