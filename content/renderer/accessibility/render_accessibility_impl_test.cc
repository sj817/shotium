...
void RenderAccessibilityImplTest::SetUp() {
  // WebTestingSupport::SaveRuntimeFeatures()/ResetRuntimeFeatures() used to
  // snapshot and restore the runtime feature flags around this fixture. Their
  // implementation lived in blink's exported layer, which is gone, so the two
  // explicit toggles below are simply left set after the test.
