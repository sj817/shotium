...
blink::mojom::CommonNavigationParamsPtr MakeCommonNavigationParams(
  ...
  // The ad legs of this policy are dead: nothing in this build can tag a
  // frame or a script as an ad. Blink's ad tagging needed either AdTracker's
  // V8 stack walk or the subresource filter, and both are gone.
  download_policy.ApplyDownloadFramePolicy(
      ...
      has_download_sandbox_flag, /*from_ad_frame=*/false,
...
void RenderFrameImpl::CommitNavigationWithParams(
  ...
  // No service worker network provider is attached: this build has no script
  // engine, so no service worker can be running to intercept this navigation
  // or its subresources. `container_info` and `controller_service_worker_info`
  // are dropped here and the document loads straight from the network.
...
void RenderFrameImpl::DidClearWindowObject() {
  TRACE_EVENT("navigation", "RenderFrameImpl::DidClearWindowObject",
              perfetto::Flow::FromPointer(this));
  // There is no JavaScript engine, so none of the `window`-object bindings
  // (WebUI, DOM automation, stats collection, gpuBenchmarking,
  // skiaBenchmarking) that used to be installed here exist any more.
  for (auto& observer : observers_) {
...
// OnMainFrameAdRectangleChanged() was here. Blink reported the on-screen
// rectangle of each ad element through it; nothing can mark an element
// ad-related any more.
...
WebView* RenderFrameImpl::CreateNewWindow(
  ...
  params->download_policy.ApplyDownloadFramePolicy(
      ...
      !GetWebFrame()->IsAllowedToDownload(), /*from_ad_frame=*/false,
      /*is_ad_script_in_stack=*/false);
