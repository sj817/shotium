...
void RendererBlinkPlatformImpl::GetWebRTCRendererPreferences(
  ...
  *ip_handling_policy =
  ...
  // The per-URL override list used to be consulted here: each entry paired a
  // ContentSettingsPattern with a policy, and the first pattern matching the
  // top frame's origin won. Both the entry struct and its mojom typemap were
  // cut -- see RendererPreferences::webrtc_ip_handling_urls -- so the global
  // policy is the only policy.
