...
int UtilityMain(MainFunctionParams parameters) {
  ...
  switch (sandbox_type) {
    ...
    // The audio and on-device-model sandbox hooks went with their services.
...
#endif  // BUILDFLAG(ENABLE_ON_DEVICE_TRANSLATION) && (BUILDFLAG(IS_LINUX) ||
        // BUILDFLAG(IS_CHROMEOS))
    // The screen AI and shape detection sandbox hooks went with their services.
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
#if BUILDFLAG(ALLOW_OOP_VIDEO_DECODER)
  ...
  // The video capture service used to raise this to kPresentation for its
  // time-sensitive frame deliveries. That service is gone.
