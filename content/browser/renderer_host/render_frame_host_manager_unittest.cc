...
// The RenderFrameHostManagerAdTaggingSignalTest suite was here, with its
// AdTaggingSimulator observer and the AdStatusInterceptingRemoteFrame that
// recorded blink.mojom.RemoteFrame::SetReplicatedIsAdFrame() calls. All five
// cases asserted that a frame's ad bit reached the right proxies. The
// replicated is_ad_frame bit, its mojom setter and
// RenderFrameHost::UpdateIsAdFrame() are all gone with ad tagging.
