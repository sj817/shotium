...
// SetIsAdFrame()/IsAdFrame() were here, along with the replicated is_ad_frame
// bit they broadcast to RemoteFrames. The only non-test caller of the entry
// point above them, RenderFrameHost::UpdateIsAdFrame(), was
// //components/subresource_filter's throttle manager, which is deleted.
