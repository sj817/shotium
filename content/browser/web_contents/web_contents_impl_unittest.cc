...
// TEST_F(WebContentsImplTest, IsLoadingExcludingAdFrames) was here. It drove
// WebContents::IsLoadingExcludingAdSubframes(), which reported the loading
// state while skipping ad-tagged subframes, and tagged its iframe with
// RenderFrameHost::UpdateIsAdFrame(). Nothing in this build can tag a frame
// as an ad, so both went with ad tagging.
