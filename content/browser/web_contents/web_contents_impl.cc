...
bool WebContentsImpl::IsAudioMuted() {
  // There is no audio output pipeline in this build, so `audio_muted_` is the
  // whole of the muting state: it records what the embedder asked for and
  // there are no streams for it to be applied to.
  return audio_muted_;
}
...
bool WebContentsImpl::HasActiveEffectivelyFullscreenVideo() {
  // No media player can be created in this build, so there is never a video
  // occupying the fullscreen element.
  return false;
}
...
base::ScopedClosureRunner WebContentsImpl::MarkAudible() {
  // Audibility was tracked by AudioStreamMonitor, which counted live audio
  // output streams. There is no audio output pipeline in this build, so this
  // hands back a runner that owns nothing.
...
bool WebContentsImpl::IsPictureInPictureAllowedForFullscreenVideo() const {
  // There is no fullscreen video to put into a picture-in-picture window.
  return false;
}
...
base::UnguessableToken WebContentsImpl::GetAudioGroupId() {
  // The group id used to identify this WebContents' audio streams to the audio
  // service. It is stable for the lifetime of the WebContents; there are no
  // streams in the group because there is no audio service.
...
int WebContentsImpl::GetCurrentlyPlayingVideoCount() const {
  // No media player can be created in this build, so no video is ever playing.
  return 0;
}

std::optional<gfx::Size> WebContentsImpl::GetFullscreenVideoSize() {
  // There is never a fullscreen video, so there is no size to report.
  return std::nullopt;
}
