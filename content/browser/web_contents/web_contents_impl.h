...
class CONTENT_EXPORT WebContentsImpl
 ...
 private:
  ...
  // The muting state requested through SetAudioMuted(). There is no audio
  // output pipeline in this build, so nothing consumes it beyond IsAudioMuted()
  // and the DidUpdateAudioMutingState observer notification.
  bool audio_muted_ = false;

  // Identifies this WebContents' (empty) audio stream group.
  const base::UnguessableToken audio_group_id_ =
