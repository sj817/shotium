...
Response PageHandler::StartScreencast(std::optional<std::string> format,
  ...
  // Screencast frames came from viz's FrameSinkVideoCapturer, whose mojom
  // carries media.mojom.VideoBufferHandle / VideoFrameInfo. media/mojo is
  // deleted, so there is no frame stream to subscribe to.
  return Response::ServerError("Screencast is not supported");
}
...
Response PageHandler::StartScreenRecording(std::optional<bool> audio,
  ...
  // MediaRecorder drove content/services/devtools_media_encoding_service, whose
  // mojom imports media/mojo/mojom/media_types.mojom. media/mojo is deleted, so
  // there is no encoder to record into.
  return Response::ServerError("Screen recording is not supported");
}

void PageHandler::StopScreenRecording(
    std::unique_ptr<StopScreenRecordingCallback> callback) {
  if (callback) {
    callback->sendFailure(Response::ServerError("No active screen recording"));
  }
}
...
void PageHandler::GetAppManifest(
  ...
  // ManifestManagerHost and the renderer-side manifest fetcher are gone with
  // the rest of the web app manifest stack, so there is nothing to ask. Fail
  // the command rather than answer with an empty manifest, which a client
  // could not tell apart from a page that really has none.
  callback->sendFailure(Response::ServerError(
      "Page.getAppManifest is not supported in this build."));
}
