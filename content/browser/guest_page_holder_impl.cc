...
bool GuestPageHolderImpl::IsAudioMuted() {
  // There is no audio output pipeline in this build, so `audio_muted_` records
  // the requested state and nothing else consumes it.
  return audio_muted_;
}
...
void GuestPageHolderImpl::SetAudioMutedFromWebContents(
  ...
  // Nothing to propagate to: the owning WebContents' muting state and ours are
  // both bookkeeping over an audio pipeline that does not exist.
}
