...
def _WriteEnvironmentBlock(path, contents):
    """Write the environment block, tolerating concurrent writers.

    `gn gen` evaluates several Windows toolchain definitions in parallel, and
    each of them runs this script for the same target cpu and writes the same
    file. Opening it directly makes whichever process arrives second fail with

        PermissionError: [Errno 13] Permission denied: 'environment.x64'

    which fails the whole generate. os.replace() alone is not enough either:
    unlike POSIX, Windows refuses to replace a destination another process
    currently holds open, so that turns into WinError 5 instead.

    Every writer produces byte-identical contents, because they are derived from
    the same vcvars output for the same cpu. So: if the file is already correct,
    do nothing -- which is the common case after the first generate and removes
    almost all of the contention. Otherwise write a private temporary and
    replace, retrying briefly while a peer holds the destination, and treating
    "a peer wrote the bytes we wanted" as success.
    """
    try:
    ...
    tmp_name = '%s.%d.tmp' % (path, os.getpid())
    ...
    # Out of retries: let the caller see the real error rather than a partial
    # or missing environment block.
    os.replace(tmp_name, path)
    ...
    for cpu in cpus:
        if cpu == target_cpu:
            ...
            if environment_block_name != '':
                ...
                _WriteEnvironmentBlock(environment_block_name, env_block)
