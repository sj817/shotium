...
const METRICS = ['p50', 'mean', 'p95'];
const PRIMARY = ['p50', 'mean'];
...
function bootstrapRatios(baseline, candidate, resamples) {
  let seed = 0x817 ^ baseline.length;
  const random = () => {
    seed ^= seed << 13; seed ^= seed >>> 17; seed ^= seed << 5;
    return (seed >>> 0) / 0x100000000;
  };
  const perMetric = {p50: [], mean: [], p95: []};
  const joint = [];
...
function bandsOf(tolerance) {
  const bands = typeof tolerance === 'number' ?
    {primary: tolerance, tail: tolerance} : {...tolerance};
  for (const key of ['primary', 'tail']) {
...
function classifyMetric(interval, tolerance) {
...
// when the median's interval still straddles the band: a page that loads
// with a wide spread of times can be a fifth quicker on average while its
// median at a hundred samples is still anyone's guess. A quicker median
// needs the mean at least within the band, so that a median win is not
// bought with a heavier tail.
//
// p95 can only make a case `slower`, and only when its whole interval is past
// the tail band. A tail whose interval merely straddles that band is the
// slowest few samples being what they always are, and does not hold up a
// median that is settled either way.
//
...
// delay, a network round trip) can only be asked not to be `slower`; making a
// 250 ms sleep quicker is not something this process's code can do.
function compare(baseline, candidate,
                 {minimumSamples = 100, resamples = 2000, tolerance = 0,
                  interval = 99} = {}) {
  ...
  const ratios = Object.fromEntries(METRICS.map(metric =>
  ...
  const intervals = Object.fromEntries(METRICS.map(metric =>
    [metric, {lo: percentile(perMetric[metric], lowerPercentile),
              hi: percentile(perMetric[metric], 100 - lowerPercentile)}]));
  const upper99 = percentile(joint, 99);

  const verdicts = Object.fromEntries(METRICS.map(metric =>
    [metric, classifyMetric(intervals[metric],
                            metric === 'p95' ? bands.tail : bands.primary)]));
  let status;
  if (baseline.length < minimumSamples) {
    status = 'insufficient-samples';
  } else if (METRICS.some(metric => verdicts[metric] === 'slower')) {
    status = 'slower';
  } else if (verdicts.mean === 'faster' ||
             (verdicts.p50 === 'faster' && verdicts.mean === 'equivalent')) {
    status = 'faster';
  ...
  return {summary, ratios, intervals, verdicts, upper99, tolerance: bands,
    status, samples: baseline.length,
    throughput: {baseline: 1000 / summary.baseline.mean,
                 candidate: 1000 / summary.candidate.mean}};
}
...
// meaningless: 2% is below any real regression this suite has caught and well
// above encoder rounding. A calibration that comes back noisier raises a
// band; one that comes back cleaner cannot lower it past this.
function calibrate(aaComparisons, {quantile = 95, floor = 0.02} = {}) {
  const margin = (result, metric) => Math.max(0, result.intervals[metric].hi - 1);
  ...
  const band = margins => margins.length ? Math.max(floor, percentile(margins, quantile)) : floor;
  return {primary: band(primary), tail: band(tail)};
}

module.exports = {percentile, distribution, compare, calibrate, bandsOf, METRICS, PRIMARY};
