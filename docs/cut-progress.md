...
### 21.10 缓存恢复了,ninja 还是从头编 —— mtime 才是它的判据

第六次:`gclient sync` 6 分钟,ninja 编到 **14,093 / 14,906**,零编译错误,被自己那个
280 分钟的上限切断。只差 813 条边加链接。

但日志里有一件不对劲的事:

```
Cache restored from key: out-shot-32137426382   (443 MB)
...
[1/14906] ACTION //build/rust/std/rules:...
```

**缓存恢复成功了,ninja 还是从第 1 条边开始编。** 原因是 ninja 判新旧只看修改时间,
而 `actions/checkout` 和 `gclient` 写下每一个文件时,时间都是「现在」。于是恢复回来的
目标文件永远比生成它的源文件旧,整棵树重编 —— 缓存等于没有。

`tools/shot/ci_stamp_mtimes.py` 给每个文件盖上**它所来自的那个版本的时间**:

- 主仓库的文件:用「最后改动它的那个提交」的时间。一次 `git log --name-only`
  就能把 67,964 个路径的时间全拿到(实测本机跑通,`DEPS` 和 `shot_renderer.cc`
  拿到的是不同的时间戳)。改十个文件的提交只推动十个时间戳,ninja 就只重编那十个。
- DEPS 管的仓库:整个仓库用它 HEAD 的提交时间。依赖是整体移动的 —— 钉住的版本一变,
  里面每个文件都可疑。
- hook 生成或下载的东西:不属于任何仓库,保留写入时间。它们是少数边的输入,
  也没有更好的说法。
- `out/` 不动:它是被比较的那一方。
...
`tools/` 下四千个浏览器工具、单元测试、`content/browser`。它们不参与编译,
所以之前的每一轮都没理由碰;但它们决定了 GitHub 上的语言构成(Java 1.5%、
Python 7.4%),让全树 ripgrep 超时,让每次上游同步多看几万个文件。
...
整体保留只删外平台子树;许可证和嵌套 `.gitignore`;用到的 Rust crate 整目录;
Blink 的 `.idl` 源),其余全删。`apply` 用显式路径列表 `git rm`,不用 `-A`。
...
每一样都是 `gn gen` 或 `ninja -n` 炸了之后才知道的,现在都在白名单里,带原因:

1. **`exec_script` 自己打开的文件。** `build/timestamp.gni` 跑
   `build/compute_build_timestamp.py`,脚本自己 `open()` 了 `../chrome/VERSION`。
   GN 记的是脚本,不是脚本读了什么。删了之后六个平台的 gn gen 全部停在
   `//build/timestamp.gni:43`。
2. **Python 的 import。** `build/win/set_appcontainer_acls.py` 往 `sys.path`
   加了 `testing/scripts`,`import common`;`common` 再 import `testing/test_env`。
   第一次全量构建的第一个失败就是 `No module named 'common'`。
3. **order-only 边。** GN 把 target 的 `inputs` 挂在 `phony/<target>.inputs` 的
   `||` 一侧,`ninja -t inputs` 不走这条边。四个 `.natvis` 就是这么丢的。
...
"not generated by a dependency"(host_os 的假象,拿不到 build.ninja),
只能靠 CI。六个平台的引擎构建在裁后的树上重新跑,mac x64 回来的图只多要
一个 `utf8_range_sse.inc`。

## 23. 构建提速:热构建归零,冷构建减半

裁完树之后看了一眼 CI:ninja 占每次运行 75–85%,而且"热"构建不热——PR2 一行
引擎代码没改,Windows 照样重编 1,318 步、67 分钟。链接不是问题(ThinLTO exe 冷
1 分 43 秒、dll 11 秒)。三件事各自都是断层级的。

### 23.1 LASTCHANGE 跟着 HEAD 走

`ninja -d explain` 给出的链:每次 CI 的 `build/util/LASTCHANGE` 都是新的 HEAD
哈希 → `base/check.cc` 的生成头变 → libbase 重打包 → 链接 libbase 的每个宿主
工具重链(protoc、nasm、perfetto 的三个 protoc 插件)→ protoc 重写 pbzero 头
(内容相同、mtime 变新,restat 救不了)→ `base/trace_event` 把这些头带进几乎
所有 Blink 单元 → 516 个 Blink TU、127 个 perfetto、72 个 net。这个值在本项目
里没有任何读者:`check.cc` 用它拼 CHECK 失败信息,而这个构建配置里那段信息
本来就被剥掉了。

修法是 DEPS 的 `lastchange` hook 把 `--filter` 钉到根提交的标题
(`^shotium: static screenshots from a stripped Chromium$`),LASTCHANGE 和
`LASTCHANGE.committime`(构建时间戳)从此是常量;`ci_stamp_mtimes` 给它 DEPS
的时间而不是 HEAD 的。

### 23.2 jumbo 按位置分块

`BUILDCONFIG.gn` 原来按 source 列表下标每 N 个切一块,单元也按下标命名。增删
一个文件,它后面所有单元的内容和名字全变,热缓存整个 target 重编——裁树那次
Windows "热"构建 1,590 步就是这么来的。现在按"相对 target 的第一级目录"分组
(`css`、`layout`、`root`、`extern`),组内每 N 个切一块,命名
`<target>_shot_jumbo_<group>_<n>.cc`。非连续出现的组用位置名兜底(2,322 个
单元里 195 个)。GN 的坑:覆盖非空 list 要先 `= []`。

### 23.3 每个 Blink 单元 14 秒是头文件的固定成本

拿 `core` 的真实编译命令在本机实测(同一条命令行,只换源文件):

```
单元大小    编译时间   每个源文件   峰值内存
 8 文件      13.9 s     1.74 s      1.84 GB
32 文件      16.4 s     0.51 s      2.17 GB
64 文件      21.1 s     0.33 s      2.49 GB
128 文件     33.2 s     0.26 s      3.38 GB
```

去掉 ThinLTO 只快 1.7 秒,所以 LTO 不是成本。合并上限 8 → 32(mac 16,那台
arm64 runner 只有 3 核 7 GB),Blink core 267 个单元变 94 个,总步骤 5,778 →
5,487。新暴露的跨文件冲突 19 处,全是已知的三类(文件级同名、`features::`
解析到别的命名空间、宏和模板特化撞上前面的文件),逐个加进目标自己的
`shot_jumbo_excluded_sources`;`net/http/http_stream_factory_job.cc` 那个不是
冲突而是缺 `#include "net/base/features.h"`,以前靠同单元的邻居带进来。
perfetto 是 DEPS checkout,BUILD.gn 改不了,为此加了全局的
...
translator。271 个对象,外加 libEGL / libGLESv2 / vulkan-1 三个 DLL 和 vulkan-loader
的 16 个对象,给一个从不碰 GPU 的二进制。`gpu/config` 之外没有任何文件 include
`gpu_util.h` / `gpu_info_collector.h` / `gpu_switching.h` / `gpu_test_config.h`;
`gpu_control_list.cc` 只用 `gpu_util` 里两个 Intel 世代函数。删掉 11 个收集器
和测试配置源文件、`//ui/gl/init` 依赖、`gpu_util.cc` 里两处 GL 初始化,步骤
5,487 → 5,221。`//ui/gl` 本身(16 个对象)另有 viz/common 和 blink public common
的路径,留着。
