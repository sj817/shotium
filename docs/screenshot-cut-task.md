...
本轮范围调整（用户最新指令，stage84开始）：只完成两组：①第三方、工具、测试、GN骨架；②Perfetto、UKM、Crashpad、性能上报。完成后集中编译并进行发版前验收，本轮不发布。Blink深层交互/AX/lifecycle与其余浏览器功能尾巴留到下次，不再主动扩展。总目标仍有未完成项，不得将本轮验收等同全局裁剪完成。优先合并大组、减少重复审计和提交；现有一个子代理仅承担Perfetto Python配套。此前拒绝的31/32删除清单、49图修复和84诊断精确修改已获得用户明确授权，其他旧保护范围仍有效。
