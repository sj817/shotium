...
def build_observers(config, files):
    ...
    # Upstream treats a probe with no observer as a mistake -- someone declared
    # instrumentation and forgot to wire it up. Here it is the normal case:
    # twenty-two of the twenty-four observers were the inspector agents and the
    # DevTools session, and they are cut. The probes stay because their call
    # sites are all over core and a probe is exactly the kind of thing that
    # comes back; with no observer each one generates an inline function whose
    # body is empty, which is what it should be.
    #
    # The check above it -- a probe named by an observer but not declared in the
    # PIDL -- is kept: that one is still a real mistake.

    for f in files:
        for probe in f.declarations:
            probe.agents = observers_by_probe.get(probe.name, set())
