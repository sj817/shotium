...
Resource::MatchStatus Resource::CanReuse(const FetchParameters& params) const {
  ...
  // The extension-scheme check compared the requesting DOMWrapperWorld with
  // the one that cached the resource. There is one world's worth of nothing
  // here, so the mismatch it guards against cannot occur.
  ...
  // Same for the service-worker variant: it exists to stop a response fetched
  // in one world from being reused in another.
