...
Resource* ResourceFetcher::MatchPreload(
  ...
  // This used to carry a TODO about keying preloads on the JavaScript world
  // the resource belongs to; there are no worlds left to distinguish.
  if (!RuntimeEnabledFeatures::PreloadLinkRelDataUrlsEnabled()) {
...
void ResourceFetcher::PopulateAndAddResourceTimingInfo(
  ...
  // Resource timing entries for resources fetched by an extension were
  // excluded here. Whether a fetch came from an extension was a property of
  // its script world, and there are none.
