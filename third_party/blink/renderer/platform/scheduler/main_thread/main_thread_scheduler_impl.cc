...
void MainThreadSchedulerImpl::PerformMicrotaskCheckpoint() {
  ...
  // This used to also drain V8's per-isolate default microtask queue,
  // which held JavaScript promise reactions. Only the
  // per-AgentGroupScheduler EventLoops below have microtasks now.
...
void MainThreadSchedulerImpl::UpdatePolicyLocked(UpdateType update_type) {
  ...
  if (new_policy.rail_mode != main_thread_only().current_policy.rail_mode) {
    // RAIL mode used to be forwarded to the isolate here. The observers below
    // are blink's own and still get it.
...
void MainThreadSchedulerImpl::DidCommitProvisionalLoad(
  ...
  if (is_outermost_main_frame && (!is_web_history_inert_commit || is_reload)) {
    ...
    // The RAIL mode was sampled either side of ResetForNavigationLocked() so
    // that a change could be pushed to V8 along with the load start timestamp.
    // There is no isolate to push it to, and the mode itself still drives the
    // scheduler's own policy from current_policy.rail_mode.
  }
