...
void MainThreadMetricsHelper::ReportLowThreadLoadForPageAlmostIdleSignal(
  ...
  // main_thread_task_load_state_ is still tracked, because the histogram
  // below is keyed on it; only the IPC telling the browser's performance
  // manager about the transition is gone.
