...
class PLATFORM_EXPORT TaskAttributionTracker {
 public:
  ...
  // The task attribution tracker for the current thread, or null if none
  // has been installed.
  //
  // This used to hang off the per-isolate embedder data, which owned one
  // tracker per isolate and created it eagerly. With no isolate to hang it
  // off, the tracker is registered per thread by whoever constructs it;
  // every caller of From() already handles a null tracker, which is what a
  // thread with no tracker installed gets.
  static TaskAttributionTracker*& CurrentForThread() {
    static thread_local TaskAttributionTracker* tracker = nullptr;
    return tracker;
  }
  static TaskAttributionTracker* From() { return CurrentForThread(); }
  static void SetForCurrentThread(TaskAttributionTracker* tracker) {
    CurrentForThread() = tracker;
  }
  ...
  // execution.
  //
  // Note: This returns std::nullopt if script was already running when
  // called, so care must be taken about ordering.
