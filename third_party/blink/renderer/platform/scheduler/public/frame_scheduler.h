 ...
 public:
  class PLATFORM_EXPORT Delegate : public FrameOrWorkerScheduler::Delegate {
   public:
    ...
    // GetDocumentResourceCoordinator() was here: the delegate's channel
    // to the browser's performance manager, used below to report WebRTC
    // usage. Cut with DocumentResourceCoordinator.
  };
  ...
  // SetIsAdFrame()/IsAdFrame() were here. LocalFrame::SetAdEvidence() was the
  // only writer and the scheduler used the bit for a trace argument only.
