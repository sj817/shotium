...
void WorkerThreadScheduler::PerformMicrotaskCheckpoint() {
  // This used to drain V8's per-isolate default microtask queue. Worker
  // microtasks now live entirely in the worker's EventLoop.
}
...
// SetV8IsolateUnused() was here. It told the isolate that its heap could be
// shrunk while the worker thread was idle.
