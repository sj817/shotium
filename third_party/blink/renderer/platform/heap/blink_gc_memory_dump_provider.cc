...
bool BlinkGCMemoryDumpProvider::OnMemoryDump(
  ...
  if (args.determinism ==
    ...
    // kForceGc asks for a dump without garbage in it. Upstream did this by
    // sending V8 a low-memory notification, which drove a unified-heap GC --
    // there was no way to collect the C++ heap on its own while the two were
    // unified. They are not unified any more: ThreadState owns a standalone
    // cppgc::Heap, so the collection can be asked for directly.
    thread_state_->CollectAllGarbageForMemoryPressure();
  }
  ...
  // Upstream read this through v8::CppHeap::CollectStatistics(), which did
  // nothing but forward to the internal HeapBase. Reaching for HeapBase from
  // here instead pulls cppgc's own src/base/logging.h into blink and redefines
  // CHECK out from under it, so the forwarder now lives in cppgc: see
  // cppgc::CollectStatistics in third_party/cppgc/v8/include/cppgc/
  // heap-statistics.h.
  ::cppgc::HeapStatistics stats = ::cppgc::CollectStatistics(
      ThreadState::Current()->heap_handle(), detail_level);
