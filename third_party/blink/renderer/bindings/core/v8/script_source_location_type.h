...
// This is a plain enum with no V8 in it. It went with bindings/core/v8 and is
// back at the same path so its ~10 include sites are unchanged. ClassicScript
// stores one per script and PendingScript/ScriptLoader branch on it, so it is
// part of how a <script> element is classified, not part of executing one.
// kEvalForScheduledAction and kInspector can no longer occur -- there is no
// setTimeout(string) and no inspector -- but the values stay: this is a
// logging vocabulary, and renumbering it buys nothing.
