...
bool CookieJar::SetCookie(const String& value) {
  ...
  backend_->SetCookieFromString(
      ...
      // RestrictedCookieManager still takes is_ad_tagged; blink can no longer
      // tag a frame as an ad, so it is always false.
      /*is_ad_tagged=*/false, apply_devtools_overrides, value);
...
String CookieJar::Cookies() {
  ...
  if (ipc_needed) {
    ...
    if (!backend_->GetCookiesString(
            ...
            document_->GetExecutionContext()->GetStorageAccessApiStatus(),
            get_version_shared_memory, /*is_ad_tagged=*/false,
