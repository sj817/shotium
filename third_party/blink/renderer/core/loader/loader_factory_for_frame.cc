...
std::unique_ptr<URLLoader> LoaderFactoryForFrame::CreateURLLoader(
  ...
  // This used to try prefetched_signed_exchange_manager_ here for a
  // matching prefetched subresource signed exchange before falling through
  // to a normal URL load. Signed exchange support is gone.
  ...
  // An `else if` arm was here: when the document is controlled by a service
  // worker, subresource loads go through that worker's network provider rather
  // than the frame's factory. Service workers are cut.
