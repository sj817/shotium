...
class CORE_EXPORT HTMLElement : public Element {
 ...
 public:
  ...
  // The <install>/<login>/<credential>/<usermedia>/<camera>/<microphone>
  // permission-prompt element family (HTMLCapabilityElementBase and its
  // subclasses) that these Is*() checks used to distinguish was removed
  // entirely: it is browser permission-prompt UI, and a screenshot engine
  // never surfaces permission prompts.
