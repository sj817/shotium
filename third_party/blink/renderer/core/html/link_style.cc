...
void LinkStyle::NotifyFinished(Resource* resource) {
  ...
  // resource->LoadFailedOrCanceled() used to report a stylesheet-loading-
  // failed issue to the DevTools Audits panel here. There is no DevTools
  // frontend and this was a pure report with no other side effect, so it is
  // simply gone.
