...
Resource* PreloadRequest::Start(Document* document) {
  ...
  // ResourceLoaderOptions used to take the current DOMWrapperWorld here, to
  // record which V8 isolated world's CSP applied to this request. There are
  // no worlds without a script engine, so ResourceLoaderOptions no longer
  ...
  if (resource_type_ == ResourceType::kScript) {
    ...
    // This used to consult LCPCriticalPathPredictor::lcp_influencer_scripts()
    // (a learned-from-previous-loads heuristic for which scripts tend to
    // delay the LCP element) to mark this preload as a likely LCP
    // influencer. LCPCriticalPathPredictor's implementation was removed
    // elsewhere in this engine, so `is_potentially_lcp_influencer_` is now
    // always left at its default (false).
  }
