...
void HTMLCanvasAccessibilityManager::SetHeuristicResult(
  ...
  // Used to also gate turning on text-run collection (for OCR fallback) on
  // base::FeatureList::IsEnabled(::features::kAccessibilityCanvas), a flag
  // declared in ui/accessibility/accessibility_features.h. That header, and
  // the rest of ui/accessibility, is gone -- there is no accessibility tree
  // to annotate and no Screen AI OCR service to invoke (see TriggerOCR()
  // below), so the flag can never be enabled and this path always takes the
...
void HTMLCanvasAccessibilityManager::UpdateAnnotation() {
  ...
  // Used to mark the canvas element dirty in the accessibility tree, forcing
  // a serialization update so AXCanvasAnnotator would notice and run OCR.
...
bool HTMLCanvasAccessibilityManager::ShouldRunOCR() const {
  // Used to first check features::GetCanvasAccessibilityMode() ==
  // kAdvanced (declared in ui/accessibility/accessibility_features.h) and
  // then whether the heuristic + text-run count suggested OCR was needed.
  // ui/accessibility is gone -- there is no accessibility tree to annotate
  // and no Screen AI OCR service to invoke (see TriggerOCR() below) -- so
  // OCR is never needed, permanently.
  return false;
}
...
void HTMLCanvasAccessibilityManager::TriggerOCR(TimerBase*) {
  ...
  // Marking the element dirty used to force an accessibility tree
  // serialization update, during which AXCanvasAnnotator would see the dirty
  // state and invoke the Screen AI OCR service. AXObjectCache is gone (no
  // accessibility tree in a screenshot renderer), so OCR is never actually
