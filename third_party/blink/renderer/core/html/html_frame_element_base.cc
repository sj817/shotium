...
void HTMLFrameElementBase::ParseAttribute(
  ...
  if (name == html_names::kSrcdocAttr) {
    ...
    // This used to also handle `onbeforeunload`, compiling its script source
    // into an EventListener via JSEventHandlerForContentAttribute. That
    // required V8, which this renderer does not have.
    HTMLFrameOwnerElement::ParseAttribute(params);
  }
}
