...
void UpdateWindowPermissionsPolicyWithDelegationSupportForClientHints(
    ...
    bool is_sync_parser) {
  // Used to warn the dev, via AuditsIssue::ReportClientHintIssue(), when a
  // client-hint meta tag looked js-injected (not http-equiv="accept-ch" and
  // not a preload-or-sync-parser-visible tag). AuditsIssue reported to the
  // DevTools Audits panel (third_party/blink/renderer/core/inspector/),
  // which was deleted wholesale; there is no front end left to warn, so the
  // check and its side-effect-free report are both gone.
  ...
  // Used to warn the dev, via AuditsIssue::ReportClientHintIssue(), when
  // invalid origins were seen in the allow list. Same reasoning as above:
  // the DevTools Audits panel this reported to no longer exists.
