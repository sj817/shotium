...
// it violates the menu's content model. This used to return a more specific
// ElementAccessibilityIssueReason (see select_mutation_observer.h for why
// that type is gone); menus never actually used more than one reason value
// here (see the removed `kPlaceholderIssue` TODO below), so a plain bool
// loses nothing.
bool MenuMutationObserver::CheckForIssue(const Node& descendant,
                                         const Node* disconnected_root,
                                         const Node* disconnected_parent) {
  ...
  if (descendant.getNodeType() == Node::kTextNode) {
    ...
    // Return true (valid) so that these detached mutations are ignored.
