...
void HTMLFrameOwnerElement::SetContentFrame(Frame& frame) {
  ...
  // The frame attach/detach pair was reported to the browser's performance
  // manager here, so it could keep its frame graph in step. Cut with
  // RendererResourceCoordinator; blink's own content_frame_ bookkeeping below
  // is unaffected.
...
void HTMLFrameOwnerElement::SetEmbeddedContentView(
  ...
  // Used to notify AXObjectCache that the children changed. AXObjectCache is
...
EmbeddedContentView* HTMLFrameOwnerElement::ReleaseEmbeddedContentView() {
  ...
  // Used to notify AXObjectCache that the children changed (when there was a
  // layout_embedded_content). AXObjectCache is gone (no accessibility tree
  // in a screenshot renderer).
...
bool HTMLFrameOwnerElement::LoadOrRedirectSubframe(
  ...
  if (url.ProtocolIsInHttpFamily()) {
    ...
    if (RuntimeEnabledFeatures::ResourceTimingInitiatorEnabled()) {
      // ResourceInitiatorHelper asked V8 for the URL of the script that
      // triggered this load, and fell back to the document URL when no script
      // was running. No script is ever running, so the fallback is the only
      // reachable answer.
      fallback_timing_info_->initiator_url = GetDocument().Url();
    }
  }
