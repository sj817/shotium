...
class SummaryDescendantsObserver : public MutationObserver::Delegate {
 ...
 private:
  ...
  void AddDescendantDisallowedErrorToNode(Element& element) {
    // This used to report a DevTools Issues panel warning
    // (AuditsIssue::ReportElementAccessibilityIssue(), core/inspector, now
    // gone) and a WebFeature UseCounter metric whenever an interactive
    // element was found inside <summary>. Unlike <select>/<menu>, <summary>
    // has no content-model violation count driving real behavior here --
    // this was purely instrumentation/reporting, so there's nothing left to
    // do.
  }
