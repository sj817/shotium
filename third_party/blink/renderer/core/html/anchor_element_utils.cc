...
bool ShouldInterveneDownloadByFramePolicy(LocalFrame* frame) {
  ...
  // Gesture-less downloads used to be intervened here when the frame was an ad
  // frame, and counted when an ad script was on the V8 stack. No frame in this
  // build can be tagged as an ad and there is no stack, so neither branch could
  // be taken; the sandbox intervention below is unaffected.
...
void AnchorElementUtils::HandleDownloadAttribute(Element* element,
  ...
  // The Navigation API used to get a say here: a navigate event was dispatched
  // and the download was abandoned if script cancelled it, then the API was
  // told the navigation had been dropped so the frame did not sit thinking it
  // was still loading. With no script and no Navigation API the event always
  // continued and the "dropped" notice had no one to notify, so both are gone
  // and the download starts unconditionally.
...
void AnchorElementUtils::EnforceBlobUrlNoopenerIfNeeded(
  ...
  if (top_level_site != blob_url_site) {
    ...
    // Used to also report this to the DevTools Audits panel via
    // AuditsIssue::ReportPartitioningBlobURLIssue(). The Audits panel
    // (third_party/blink/renderer/core/inspector/) was deleted wholesale;
    // there is no DevTools front end left to receive the issue, so the
    // report is gone. The actual behavioral side effect above (forcing
    // noopener on the navigation) and the UseCounter metric are unaffected.
  }
}
