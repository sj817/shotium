...
bool BaseTextInputType::PatternMismatchPerValue(const StringView& value) const {
  ...
  if (!regexp_ || pattern_for_regexp_ != raw_pattern) {
    auto compiled = std::make_unique<PatternRegexp>(raw_pattern);
    if (!compiled->IsValid()) {
      GetElement().GetDocument().AddConsoleMessage(
          MakeGarbageCollected<ConsoleMessage>(
              ...
              StrCat({"Pattern attribute value ", raw_pattern,
                      ...
                      compiled->ExceptionMessage()})));
    }
    regexp_ = std::move(compiled);
    pattern_for_regexp_ = raw_pattern;
  }
  // A pattern that does not compile imposes no constraint, matching the
  // behaviour when the pattern was compiled by the script engine.
  if (!regexp_->IsValid()) {
    return false;
  }
  return !regexp_->MatchesEntirely(value.ToString());
}
