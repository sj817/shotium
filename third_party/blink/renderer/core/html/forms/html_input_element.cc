...
void HTMLInputElement::ParseAttribute(
  ...
  // `onsearch` used to be handled here: it compiled its script source into
  // an EventListener via JSEventHandlerForContentAttribute, which required
  // V8. There is no script engine in this renderer, so it is no longer
  // parsed specially.
...
void HTMLInputElement::SetChecked(bool now_checked,
  ...
  // This used to notify AXObjectCache of the checked-state change here
  // (ideally we'd have done it from the layout tree, matching
  // LayoutTextView, but it wasn't possible to do it at the time because of
  // the way the code is structured). AXObjectCache is gone (no accessibility
  // tree in a screenshot renderer).
...
void HTMLInputElement::setIndeterminate(bool new_value) {
  ...
  // Used to notify AXObjectCache of the checked-state change. AXObjectCache
  // is gone (no accessibility tree in a screenshot renderer).
}
