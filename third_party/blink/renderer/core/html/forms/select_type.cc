 ...
 public:
  ...
  PopoverHideResult HidePopoverInternal(
    ...
    if (auto* select = ParentSelect()) {
      ...
      // Used to notify AXObjectCache that the menu list popup was hidden.
      // AXObjectCache is gone (no accessibility tree in a screenshot
      // renderer).
    }
...
void MenuListSelectType::UpdateTextStyleAndContent() {
  ...
  // Used to notify AXObjectCache that the box's text changed. AXObjectCache
...
void MenuListSelectType::DidUpdateActiveOption(HTMLOptionElement* option) {
  // This entire notification (tracking the previously-active option and
  // reporting the change via AXObjectCache::MarkElementDirty() /
  // HandleUpdateActiveMenuOption()) existed purely to keep the
  // accessibility tree's "active option" in sync. AXObjectCache is gone (no
  // accessibility tree in a screenshot renderer), so there's nothing left to
  // notify.
}
...
void ListBoxSelectType::ScrollToSelection() {
  ...
  // Used to notify AXObjectCache of the listbox active index. AXObjectCache
  // is gone (no accessibility tree in a screenshot renderer).
}
