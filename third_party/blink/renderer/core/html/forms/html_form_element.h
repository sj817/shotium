...
class CORE_EXPORT HTMLFormElement final : public HTMLElement {
 ...
 public:
  ...
  // attributes exposing a <form> as an AI-agent-invokable tool) required V8
  // to run agent-supplied script and has been removed along with it. These
  // two accessors back CSS pseudo-classes (`:tool-form-active`,
  // `:tool-submit-active`) that selector_checker.cc still queries; since no
  // tool can ever be active in this engine, they honestly always report
  // "not active" rather than being deleted out from under the CSS code.
  bool IsActiveToolSubmitButton(const HTMLFormControlElement* element) const {
    return false;
  }
  bool MatchesToolFormActivePseudoClass() const { return false; }

  // Always empty: there is no declarative WebMCP tool invocation mechanism
  // left to report an id for. See form_submission.cc's use of this.
  std::optional<base::UnguessableToken> GetActiveWebMCPToolInvocationId()
      const {
    return std::nullopt;
  }
 ...
 private:
  ...
  // The declarative WebMCP tool (`HTMLFormMcpTool`, which registered `this`
  // as an AI-agent-invokable tool via `DeclarativeWebMCPTool`) and its
  // supporting registration/scheduling machinery (`active_webmcp_tool_`,
  // `mcp_registration_task_`, `ScheduleDeclarativeWebMCPToolRegistration()`,
  // `RegisterDeclarativeWebMCPTool()`, `ReportInvalidMCPFormIssueIfNeeded()`,
  // `ScheduleWebMCPSchemaUpdateIfActive()`) were removed: `DeclarativeWebMCPTool`
  // required V8 to invoke agent-supplied JavaScript, which this engine does
  // not have.
