...
void SelectMutationObserver::AddDescendantDisallowedErrorToNode(
  ...
  bool is_valid_child = CheckForIssue(node);
  if (!is_valid_child) {
    if (!IsAllowedInteractiveElement(node)) {
      select_->IncreaseContentModelViolationCount();
    }
    // This used to also report the specific disallowed-content reason via
    // AuditsIssue::ReportElementAccessibilityIssue() (a DevTools Issues
    // panel warning) and RecordIssueByType() (a WebFeature UseCounter
    // metric). AuditsIssue (core/inspector) is gone -- it's browser
    // DevTools UI, and a screenshot engine has no DevTools front end to show
    // issues to. The content-model violation counting above, which drives
    // <select>'s dialog-mode role switching, is the only load-bearing part
    // and is kept.
  }
}
...
bool SelectMutationObserver::CheckForIssue(const Node& descendant) {
  ...
  if (IsA<HTMLSelectElement>(*parent)) {
    return IsAllowedDescendantOfSelect(descendant, *parent);
  }
  if (IsA<HTMLOptGroupElement>(*parent)) {
    return IsAllowedDescendantOfOptgroup(descendant, *parent);
  }
  ...
  if (IsA<HTMLButtonElement>(*parent)) {
    return IsAllowedDescendantOfButton(descendant);
  }
  if (IsA<HTMLLegendElement>(*parent)) {
    return IsAllowedPhrasingContent(descendant) &&
...
bool SelectMutationObserver::CheckDescedantOfOption(const Node& descendant) {
  ...
  return !HasTabIndexAttribute(descendant) && !IsContenteditable(descendant);
}
...
bool SelectMutationObserver::TraverseAncestorsAndCheckDescendant(
    const Node& descendant) {
