...
class CORE_EXPORT HTMLSubmitButtonBehavior final : public ElementBehavior {
 ...
 public:
  // Identifies this behavior type. ElementInternals both de-duplicates
  // behaviors and looks them up by this name.
  static constexpr const char* kBehaviorName = "HTMLSubmitButtonBehavior";
...
struct blink::DowncastTraits<blink::HTMLSubmitButtonBehavior> {
  static bool AllowFrom(const blink::ElementBehavior& behavior) {
    return std::string_view(behavior.BehaviorName()) ==
           blink::HTMLSubmitButtonBehavior::kBehaviorName;
  }
};
