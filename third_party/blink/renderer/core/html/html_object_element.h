...
class CORE_EXPORT HTMLObjectElement final : public HTMLPlugInElement,
 ...
 public:
  ...
  String data();
  String codeBase();
  // setData()/setCodeBase() used to live here: IDL setters taking a
  // (TrustedScriptURL or USVString) and Trusted-Types-checking it before
  // reflecting to the `data`/`codeBase` content attribute. They were only
  // ever reachable from script (`object.data = ...`), and this engine has no
  // script engine to call them, so they were removed.
