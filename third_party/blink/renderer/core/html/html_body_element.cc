...
void HTMLBodyElement::ParseAttribute(
  ...
  if (name == html_names::kVlinkAttr || name == html_names::kAlinkAttr ||
    ...
    // This used to also handle `<body>`'s window-forwarded event handler
    // content attributes (onload, onerror, onresize, onscroll, ...): each
    // `on*` attribute compiled its script source into an EventListener via
    // JSEventHandlerForContentAttribute and attached it to `window` (or, for
    // onselectionchange, to the document). That required V8. There is no
    // script engine in this renderer, so those attributes can never produce
    // a listener and are no longer parsed specially here.
