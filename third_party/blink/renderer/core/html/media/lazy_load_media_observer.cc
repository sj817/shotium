...
void LazyLoadMediaObserver::LoadIfNearViewport(
  ...
  for (auto entry : entries) {
    ...
    // This used to also resume a deferred <video>/<audio> resource-selection
    // load and poster fetch here (HTMLMediaElement::LoadDeferredMediaIfNeeded).
    // HTMLMediaElement no longer defers either: there is no WebMediaPlayer
    // for a resource load to feed, and the poster's own lazy-load gate
    // (ShouldDeferMediaLoad(), core/loader/lazy_media_helper.cc) required
    // script execution to even start monitoring, which never happens in a
    // build with no V8 -- so no media element is ever added to
    // `lazy_load_intersection_observer_` in the first place. See
    // core/html/media/html_video_element.cc's UpdatePosterImage() comment.
