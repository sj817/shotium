...
gfx::Rect ComputeControlsRect(const LayoutObject& container) {
  // This used to walk from the LayoutVTTCue's TextTrackContainer up to the
  // HTMLMediaElement, then ask its MediaControls for the button-panel and
  // timeline boxes, so cues could avoid overlapping the native controls.
  // MediaControls no longer exists in this build (there is no player for it
  // to control), so there is no controls area to avoid.
