...
const char* HTMLElement::GetHumanReadableName() const {
  // This used to build a full "<tag attr=...>" start-tag string via
  // MarkupAccumulator when a V8 heap snapshot was in progress, so the
  // devtools heap profiler could label each element node with its markup.
  // The V8 heap profiler (and V8 itself) is gone, so a snapshot is never in
  // progress and this is permanently the tag-name-only fallback.
...
void HTMLElement::ParseAttribute(const AttributeModificationParams& params) {
  ...
  // This used to compile the `on*` content attribute's value (e.g.
  // `onclick="..."`) into a callable EventListener via
  // JSEventHandlerForContentAttribute, which required V8 to run the
  // resulting script. There is no script engine in this renderer, so
  // content-attribute event handlers can never fire and are no longer wired
  // up; `triggers->event` is left unused here as a result.
...
namespace {
// This used to mark *all* invokers for the given popover dirty in the AX
// tree via AXObjectCache, since they all should now have an updated
// expanded state. AXObjectCache is gone (no accessibility tree in a
// screenshot renderer), so there's nothing left to notify.
...
PopoverHideResult HTMLElement::CloseEntirePopoverStack(
  ...
  // This used to collect popovers HidePopoverInternal() reported as kept
  // open on behalf of an attached DevTools session
  // (kForcedOpenByInspector), pop them off `stack` to avoid looping forever
  // on them, and put them back afterward. HidePopoverInternal() can no
  // longer return that value (see its definition), so every popover really
  // does hide -- which, as a side effect, erases itself from `stack` -- and
  // this always returns kHidden.
  while (!stack.empty()) {
...
PopoverHideResult HTMLElement::HideAllPopoversUntil(
  ...
  // The `popovers_held_open_by_inspector` out-param and the DevTools-forced-
  // open bookkeeping that used to fill it in are gone; see the comment in
  // HidePopoverInternal(). It is left in the signature (unused) because
  // external callers (fullscreen.cc, html_dialog_element.cc) still pass the
  // default and code outside this file still reads the declaration.
  auto hide_stack_until = [&find_last_to_hide, &focus_behavior,
    ...
    do {
      ...
      while (last_to_hide && last_to_hide->popoverOpen()) {
        CHECK(!stack.empty());
        // Hiding stack.back() erases it from `stack` as a side effect, so
        // this loop terminates once last_to_hide itself has been hidden.
        stack.back()->HidePopoverInternal(
            /*invoker=*/nullptr, focus_behavior, transition_behavior,
            exception_state);
      }
...
PopoverHideResult HTMLElement::HidePopoverInternal(
    ...
    HidePopoverTransitionBehavior transition_behavior,
    ExceptionState* exception_state) {
  // This used to call probe::WillHidePopover() to let an attached DevTools
  // Elements-panel session veto the hide and keep the popover open
  // (PopoverHideResult::kForcedOpenByInspector), e.g. so the user could
  // inspect it. There is no DevTools frontend and no probe agent left that
  // implements WillHidePopover, so `force_open` could never become true;
  // this is now permanently the "not forced open" path, i.e. every one of
  // the kForcedOpenByInspector branches below and in CloseEntirePopoverStack
  // / HideAllPopoversUntil is unreachable dead code, kept only where
  // deleting it would require rewriting the surrounding stack bookkeeping.
...
void HTMLElement::AddHTMLBackgroundImageToStyle(
  ...
  auto* image_value =
      MakeGarbageCollected<CSSImageValue>(*MakeGarbageCollected<CSSUrlData>(
          ...
          /*origin_clean=*/true, /*modifiers=*/CSSUrlRequestModifiers()));
  if (initiator_name) {
