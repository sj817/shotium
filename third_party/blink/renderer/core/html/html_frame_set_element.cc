...
void HTMLFrameSetElement::ParseAttribute(
  ...
  if (name == html_names::kRowsAttr) {
    ...
    // This used to also handle `<frameset>`'s window-forwarded event handler
    ...
    // JSEventHandlerForContentAttribute and attached it to `window`. That
    // required V8. There is no script engine in this renderer, so those
    // attributes can never produce a listener and are no longer parsed
    // specially here.
