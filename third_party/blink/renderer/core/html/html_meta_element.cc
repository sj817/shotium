...
void HTMLMetaElement::ProcessContent() {
  ...
  if (EqualIgnoringAsciiCase(name_value, "viewport")) {
    ...
    // This used to compare the referrer policy before/after the call above
    // and, on change, tell DocumentSpeculationRules to re-evaluate
    // <script type=speculationrules> rules that inherit the document
    // referrer policy. speculationrules is a scripting feature and
    // DocumentSpeculationRules is gone with script, so the before/after
    // GetReferrerPolicy() reads (which existed only to feed that
    // comparison) are gone too.
