...
// anchor_element_interaction_tracker.h is gone: it told the browser process
// to preconnect to a link the pointer was heading for, but there is no
// browser process and no second navigation to speed up. Its one call site
// below (in HandleClick()) is gone too.
...
void HTMLAnchorElementBase::ParseAttribute(
  ...
  if (params.name == html_names::kHrefAttr) {
    ...
    // Used to notify DocumentSpeculationRules so <script type=speculationrules>
    // href-matching rules could react to the href change. speculationrules is
    // a scripting feature and DocumentSpeculationRules is gone with script.
    ...
    // Used to notify DocumentSpeculationRules of a rel change; gone with
    // script (see the kHrefAttr branch above).
  } else if (params.name == html_names::kReferrerpolicyAttr) {
    // This branch used to notify DocumentSpeculationRules of a
    // referrerpolicy change; gone with script (see the kHrefAttr branch
    // above). The branch is kept, empty, so this attribute still does not
    // fall through to HTMLElement::ParseAttribute() below, matching the
    // pre-existing (pre-speculation-rules) behavior for this attribute.
  } else if (params.name == html_names::kTargetAttr) {
    // Same as kReferrerpolicyAttr above: used to notify
    // DocumentSpeculationRules of a target change, now gone with script.
  } else {
...
void HTMLAnchorElementBase::HandleClick(MouseEvent& event) {
  ...
  // Used to tell the AnchorElementInteractionTracker about this click so it
  // could refine which preconnect predictions paid off. The tracker (and
  // the browser-process preconnecting it fed) is gone; see
