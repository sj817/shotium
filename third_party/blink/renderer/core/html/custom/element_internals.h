...
class CORE_EXPORT ElementInternals : public ScriptWrappable,
 ...
 private:
  ...
  void SetBehaviors(HeapVector<Member<ElementBehavior>> behaviors,
                    ExceptionState& exception_state);
  // Looks up an attached behavior by its ElementBehavior::BehaviorName() token,
  // which SetBehaviors() already treats as the behavior's type identity.
  ElementBehavior* FindBehaviorByName(std::string_view behavior_name) const;
