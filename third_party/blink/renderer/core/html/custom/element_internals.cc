...
void ElementInternals::setAttribute(const QualifiedName& attribute,
  ...
  // Used to notify AXObjectCache of the attribute change. AXObjectCache is
  // gone (no accessibility tree in a screenshot renderer); the semantics map
  // above is still maintained for anything that reads it directly.
}
...
ElementBehavior* ElementInternals::FindBehaviorByName(
    std::string_view behavior_name) const {
  CHECK(RuntimeEnabledFeatures::ElementInternalsBehaviorsEnabled());
  if (!behaviors_) {
    return nullptr;
  }
  for (ElementBehavior* behavior : behaviors_->AsVector()) {
    if (std::string_view(behavior->BehaviorName()) == behavior_name) {
      return behavior;
    }
  }
  return nullptr;
