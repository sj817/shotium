...
HTMLImageElement::HTMLImageElement(Document& document, bool created_by_parser)
  ...
  // LCPScriptObserver and creator_scripts_ removed in this cut -- see the
  // comment on creator_scripts() in html_image_element.h.
}
...
LayoutObject* HTMLImageElement::CreateLayoutObject(const ComputedStyle& style) {
  ...
  switch (layout_disposition_) {
    ...
    case LayoutDisposition::kPrimaryContent: {
      auto* image = MakeGarbageCollected<LayoutImage>(this);
...
Node::InsertionNotificationRequest HTMLImageElement::InsertedInto(
  ...
  // LCPCriticalPathPredictor and LocalFrame::GetLCPP() were removed in this
  // cut (see local_frame.h): the predictor lived under the deleted
  // chrome-facing LCPP module and was fed by browser-process hints over a
  // deleted mojo interface. A static screenshot engine never gets those
  // cross-navigation hints, so there is no locator match to check here.
  ...
  if (!is_lazy_load_issue_reported_ && HasLazyLoadingAttribute()) {
    if (LayoutImage* layout_image = DynamicTo<LayoutImage>(GetLayoutObject())) {
      if (layout_image->IsUnsizedImage() || LayoutBoxWidth() == 0 ||
          LayoutBoxHeight() == 0) {
        // This used to compute the image's URL (currentSrc(), falling back
        // to ImageSourceURL()) and report a lazy-load-image issue to the
        // DevTools Audits panel. There is no DevTools frontend and this was
        // a pure report with no other side effect, so the URL computation
        // and the report are both gone; is_lazy_load_issue_reported_ still
        // latches so this check runs at most once per element, matching the
        // original behavior.
