...
void SlotAssignment::RecalcAssignment() {
    ...
    return;
  {
    ...
    // Used to invalidate the accessibility cache for any <slot> within this
    // shadow root before flat tree traversal is forbidden (any <slot> may
    // lose or gain flat tree children during slot reassignment); no
    // accessibility tree exists to invalidate anymore.
    ...
    if (owner_->IsManualSlotting()) {
      ...
      for (Member<HTMLSlotElement>& slot : Slots()) {
        for (Node* slottable : slot->ManuallyAssignedNodes()) {
          ...
          if (slottable && slottable->IsChildOfShadowHost() &&
            ...
            // Recomputing the a11y subtree on tree-scope change was here;
            // no accessibility tree exists to recompute anymore.
          }
        }
      }
      ...
      for (Node& child : NodeTraversal::ChildrenOf(owner_->host())) {
        ...
        if (HTMLSlotElement* slot = FindSlotByName(child.SlotName())) {
          ...
          // Recomputing the a11y subtree on tree-scope change was here; no
          // accessibility tree exists to recompute anymore.
        } else {
          child.ClearFlatTreeNodeData();
          child.RemovedFromFlatTree();
        }
