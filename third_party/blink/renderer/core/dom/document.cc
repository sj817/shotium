...
NodeIterator* Document::createNodeIterator(Node* root,
  ...
  DCHECK(root);
  return MakeGarbageCollected<NodeIterator>(root, what_to_show);
}

TreeWalker* Document::createTreeWalker(Node* root,
                                       unsigned what_to_show) {
  DCHECK(root);
  return MakeGarbageCollected<TreeWalker>(root, what_to_show);
}
...
void Document::ScheduleLayoutTreeUpdate() {
  ...
  // DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT_WITH_CATEGORIES(...) was here.
  ++style_version_;
}
...
void Document::UpdateStyleAndLayoutTreeForThisDocument() {
  ...
  // blink rather than to script. There is no profiler and no isolate.
  ...
  TRACE_EVENT_BEGIN("blink,devtools.timeline", "UpdateLayoutTree");
  ...
  // DocumentSpeculationRules::DocumentStyleUpdated() was called here: a style
  // update can change which links match a document rule's selector.
...
void Document::ApplyScrollRestorationLogic() {
  ...
  // The annotation container attached its pending annotations here, once the
  // lifecycle was clean enough to search the rendered text for them. Text
  // annotations are created by the browser (shared highlights, "find in page"
  // style markers) and that whole family is gone; the fragment anchor below,
  // which is what scrolls a document to "#id", is unaffected.
...
void Document::Shutdown() {
  ...
  // TextFragmentHandler was told the document was going away so it could drop
...
// AXObjectCacheOwner(), ComputeAXModeFromAXContexts(), AddAXContext(),
// AXContextModeChanged(), RemoveAXContext(), ClearAXObjectCache(),
// ExistingAXObjectCache() and RefreshAccessibilityTree() built, tracked the
// AXMode of, and tore down the per-document AXObjectCache. A static
...
DocumentParser* Document::OpenForNavigation(
  ...
  // An AnchorElementInteractionTracker was attached here so that pointer
  // movement toward a link could tell the browser to start preconnecting.
  // There is no browser process and no second navigation to speed up; the
  // tracker and its call sites in core/input are gone.
  return parser;
}
...
void Document::DispatchHandleLoadStart() {
  // Used to notify the AXObjectCache that loading started; no accessibility
  // tree exists to notify anymore.
}

void Document::DispatchHandleLoadComplete() {
  // Used to notify the AXObjectCache that loading completed; no
...
bool Document::CheckCompletedInternal() {
  ...
  if (!Loader()->SentDidFinishLoad()) {
    ...
    // DetectJavascriptFrameworksOnLoad() used to run here. It identified which
    // JS framework a page was built with by probing the script global object,
    // purely to report a UKM metric. There is no script global object to probe.
  ...
  // The LCP critical path predictor was told the outermost main frame had
  // finished loading, so it could record which subresources mattered for the
  // next visit. It predicts across page loads; this process renders one.
...
void Document::SetURL(const KURL& url) {
  ...
  // FragmentDirective::ConsumeFragmentDirective() stripped the ":~:text=..."
  // tail off the fragment here and kept it for the text-fragment anchor to
  // match against. Scroll-to-text-fragment is gone (see
  // core/page/scrolling/fragment_anchor.cc), so the directive is left in the
  // fragment, where ElementFragmentAnchor already ignores anything past ":~:".
...
void Document::UpdateBaseURL() {
  ...
  // DocumentSpeculationRules::DocumentBaseURLChanged() was called here: rule
  // URLs are resolved against the base URL.
}
...
void Document::ProcessBaseElement() {
  ...
  if (old_base_target != base_target_) {
    // DocumentSpeculationRules::DocumentBaseTargetChanged() was called here:
    // a document rule can select on a link's effective target.
  }
}
...
bool Document::HasDocumentPictureInPictureWindow() const {
  // See TreeScope::pictureInPictureElement(): there is no picture-in-picture
  // window for this document to own.
  return false;
}
...
String Document::cookie(ExceptionState& exception_state) const {
  ...
  // Reading document.cookie synchronously was reported to the DevTools Audits
  // panel as a performance issue. There is no panel to report to, and the
  // UseCounter above still records the read.
...
void Document::FinishedParsing() {
  ...
  // The WebMCP model context (the Model Context Protocol surface a page uses
  // to expose tools to an agent) was told parsing had finished so it could
  // publish them. It is a script-facing supplement and went with modules.
...
// GetResourceCoordinator() was here. It lazily bound a
// performance_manager.mojom.DocumentCoordinationUnit and reported lifecycle
// state, network idleness and ad-frame status to the browser's performance
// manager. Nothing in the renderer read any of it back.
...
bool Document::IsFocusAllowed(FocusTrigger trigger,
  ...
  // These counters were also split on LocalFrame::IsAdFrame(). Nothing can tag
  // a frame as an ad, so only the NotAdFrame variants are reachable.
  uma_type = sandboxed
...
void Document::OnWarnUnusedPreloads(Vector<KURL> unused_preloads) {
  // The unused-preload list was handed to the LCP critical path predictor so
  // the next load of this page would not preload them again. Nothing predicts
  // across loads here; the warning itself is still emitted by the caller.
}
