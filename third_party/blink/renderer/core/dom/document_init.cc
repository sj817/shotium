...
Document* DocumentInit::CreateDocument() const {
  ...
  switch (type_) {
    ...
    case Type::kMedia:
      ...
      // WebMediaPlayer left to play or control (see core/html/media's file
      // comments), so that wrapper would show nothing; fall back to a plain
      // HTMLDocument rather than keep a class whose only job was building a
      // player UI that can no longer do anything.
      return MakeGarbageCollected<HTMLDocument>(*this);
    case Type::kSVG:
