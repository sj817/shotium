...
TreeWalker::TreeWalker(Node* root_node,
    ...
    : NodeIteratorBase(root_node, what_to_show), current_(root()) {}
...
Node* TreeWalker::parentNode(ExceptionState& exception_state) {
  ...
  while (node != root()) {
    ...
    if (exception_state.HadException())
      return nullptr;
    if (accept_node_result == kFilterAccept)
...
Node* TreeWalker::TraverseSiblings(ExceptionState& exception_state) {
  ...
  while (true) {
    ...
    while (sibling) {
      ...
      // type is previous.
      if (result == kFilterReject || !sibling)
...
Node* TreeWalker::previousNode(ExceptionState& exception_state) {
  ...
  while (node != root()) {
    while (Node* previous_sibling = node->previousSibling()) {
      ...
      if (exception_state.HadException())
        return nullptr;
      if (accept_node_result == kFilterReject)
        continue;
...
Children:
  ...
  while (Node* next_sibling =
    ...
    if (accept_node_result == kFilterAccept)
      return SetCurrent(node);
    if (accept_node_result == kFilterSkip)
      goto Children;
  }
  return nullptr;
}
