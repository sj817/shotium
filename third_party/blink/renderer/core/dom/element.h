...
class CORE_EXPORT Element : public ContainerNode {
 ...
 public:
  ...
  // The Trusted Types overloads taking a `const V8TrustedType*` were removed
  // with V8. A TrustedHTML/TrustedScript/TrustedScriptURL is a wrapper a
  // page mints from script to vouch for a string; with no script engine
  // nothing can mint one and nothing can pass one in. The AtomicString
  // overloads beside these are what markup reaches.
  ...
  //
  // Returns the custom element registry associated with this element: the
  // element's own scoped registry if it has one, otherwise its tree scope's.
  ...
  // SetIsAdRelated()/IsAdRelated()/GetAdProvenance()/ShouldHighlightAd() were
  // here. They hung a DisplayAdElementMonitor off the element's rare data to
  // record that an ad script or a filterlist rule produced this element, and
  // told the painter whether to draw the "Highlight ads" overlay. Both
  // producers are gone -- AdTracker read the V8 stack, and nothing implements
