...
class DisplayLockStyleScope {
 ...
 public:
  ...
  ~DisplayLockStyleScope() {
    if (auto* context = element_->GetDisplayLockContext()) {
      if (did_update_children_) {
        ...
        // DocumentSpeculationRules::DidStyleChildren() was called here.
      }
    }
  }
  ...
  void NotifyChildStyleRecalcWasBlocked(const StyleRecalcChange& change) {
    ...
    // DocumentSpeculationRules::ChildStyleRecalcBlocked() was called here.
  }
...
void Element::SetElementAttribute(const QualifiedName& name, Element* element) {
  ...
  if (isConnected()) {
    // Used to notify the AXObjectCache of the attribute change; no
...
void Element::scrollByForTesting(double x, double y) {
  ...
  if (GetDocument().ScrollingElementNoLayout() == this) {
    ScrollFrameBy(scroll_to_options, nullptr);
  } else {
    ScrollLayoutBoxBy(scroll_to_options, nullptr);
  }
}
...
// computedRole()/computedName() (and their NoLifecycleUpdate variants) used
// to spin up a throwaway AXContext/AXObjectCache to compute the ARIA
// role/accessible name for this element. There is no accessibility tree to
// query anymore, and no algorithm to compute one from without it, so these
// now honestly report "unknown".
...
void Element::NotifyAXOfAttachedSubtree() {
  // Used to notify the AXObjectCache that this subtree was attached; no
  // accessibility tree exists to notify anymore.
}
...
void Element::Focus(const FocusParams& params) {
  ...
  // Callers that focus on behalf of another frame set it explicitly. Internal
  // C++ callers (dialog.showModal, reportValidity, etc.) leave it null but they
  // always focus within their own document, so the element's frame is the
  // correct focus setter.
...
PseudoElement* Element::UpdatePseudoElement(
  ...
  if (change.ShouldUpdatePseudoElement(*element)) {
    ...
    if (generate_pseudo) {
      // Used to notify the AXObjectCache to remove this pseudo element's
      // subtree before regenerating it; no accessibility tree exists to
      // notify anymore.
