...
DispatchEventResult EventDispatcher::Dispatch() {
  ...
  if (is_click && event_->isTrusted() && frame) {
    // kAdClick counted trusted clicks inside ad frames, guarded by a DCHECK
    // that no ad script was on the V8 stack. Neither the frame tagging nor the
...
inline void EventDispatcher::DispatchEventPostProcess(
  ...
  if (is_click) {
    // Used to fire an accessibility event indicating a node was clicked on;
    // no accessibility tree exists to notify anymore.
