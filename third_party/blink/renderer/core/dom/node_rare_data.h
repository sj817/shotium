...
class CORE_EXPORT NodeRareData final : public GarbageCollected<NodeRareData> {
 ...
 private:
  enum class FieldId : unsigned {
    ...
    // 39 was kDisplayAdElementMonitor, which held the ad-tagging state for an
    // element. The values are bit positions in a per-object bitfield and are
    // never persisted, so the gap costs nothing and renumbering the rest would
    // only churn an UNSAFE_BUFFERS-indexed array.
