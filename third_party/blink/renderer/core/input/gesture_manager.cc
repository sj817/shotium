...
WebInputEventResult GestureManager::HandleGestureTap(
  ...
  if (RuntimeEnabledFeatures::TextFragmentTapOpensContextMenuEnabled() &&
    ...
    current_hit_test = event_handling_util::HitTestResultInFrame(
        frame_, HitTestLocation(adjusted_point), hit_type);
    // A tap landing on a shared-highlight annotation opened the context menu
    // instead of being treated as an ordinary tap. AnnotationAgentImpl is the
    // browser-driven text annotation machinery and is gone, so no hit test can
    // be over an annotation.
  }
