...
void DisplayLockUtilities::ScopedForcedUpdate::Impl::Destroy() {
  ...
  // The forced-update roots were collected here and handed to
  // DocumentSpeculationRules, so that links revealed by unlocking a
  // content-visibility subtree could be re-matched against the document rules.
  // Speculation rules are cut, so only the scope-ended notification remains.
  for (auto context : forced_context_set_) {
...
Element* DisplayLockUtilities::LockedAncestorPreventingPrePaint(
  ...
  // reachable when accessibility support (AXObjectCache) is enabled. The
  // accessibility tree was removed, so that special case (looking past the
  // canvas at its parent) never applies anymore.
  return result;
}
