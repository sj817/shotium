...
// when their script forced rendering of a content-visibility:hidden
// subtree. That warning was only ever issued from
// DisplayLockContext::UpgradeForcedScope() when script (a v8::Isolate) was
// the one doing the forcing; V8 is gone from blink core/platform, so there
// is no script left to force anything and the whole warning path -- this
// namespace, the field below, and the method -- is gone with it.
...
// DisplayLockDocumentState::IssueForcedRenderWarning() used to live here;
// see the comment above the namespace block near the top of this file for
// why it and its supporting state were removed.
