...
class CORE_EXPORT DisplayLockDocumentState final
 ...
 public:
  ...
  // Used to declare IssueForcedRenderWarning(Element*), which warned devs
  // when script forced rendering of a content-visibility:hidden subtree.
  // With V8 gone there is no script left to do the forcing, so the warning
  // (and the console-message state backing it) was deleted; see
