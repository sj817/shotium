...
void DisplayLockContext::Lock() {
  ...
  // Used to notify the AXObjectCache to update |element_|'s children; no
...
void DisplayLockContext::UpgradeForcedScope(ForcedPhase old_phase,
  ...
  if (IsLocked()) {
    ...
    // i.e. warn only when script is what's forcing the render, via
    // Agent::isolate() (the v8::Isolate). Agent no longer has an isolate --
    // V8 is gone from blink core/platform -- and with it every caller that
    // could force this update from script. There is no longer any script to
    // attribute the warning to, so the console warning about JS forcing a
    // hidden content-visibility:auto subtree to render is permanently moot.
  }
}
...
void DisplayLockContext::Unlock() {
  ...
  // Used to notify the AXObjectCache to remove |element_|'s subtree; no
  // accessibility tree exists to notify anymore.
...
void DisplayLockContext::ElementDisconnected() {
  ...
  // DocumentSpeculationRules was told the element had been disconnected here,
  // so that any document-rule link inside it stopped being a prefetch
  // candidate. Speculation rules are cut.
...
bool DisplayLockContext::IsScreenReaderActive() const {
  // Used to ask the AXObjectCache whether a screen reader was attached; a
  // static-screenshot engine never has one.
  return false;
}
