...
void RouteMap::EstablishNavigationStateFromActivation() {
  // This read `domWindow()->navigation()->activation()` -- the Navigation API's
  // description of which history entry this document was activated from -- and
  // used the to/from entries and the navigation type to seed NavigationState
  // for a cross-document view transition.
  //
  // core/navigation_api is cut, so there is no activation object. Upstream's
  // own first statement after fetching it is
  //
  //     if (!activation) { return; }
  //
  // which is exactly the state this engine is permanently in: it performs one
  // navigation and is never activated from a previous entry. Same-document
  // navigation state is still established by SetCommitted() and the rest of
  // this class.
}
