...
class CORE_EXPORT TaskAttributionTrackerImpl
 ...
 public:
  static std::unique_ptr<TaskAttributionTracker> Create();
  ...
  // "is JavaScript running" check when propagating task state. This is
 ...
 private:
  ...
  // A `v8::Isolate* isolate_` was here. This class used to be owned by, and
  // scoped to the lifetime of, the isolate; it registered a promise hook on
  // it (see OnStart()/OnStop() in the .cc) to trace task attribution through
  // promise reaction chains. There is no isolate any more, so both the
  // member and the hook are gone -- see task_attribution_tracker_impl.cc.
