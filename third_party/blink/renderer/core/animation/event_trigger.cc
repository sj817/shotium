...
EventTrigger* EventTrigger::Create(ExecutionContext* execution_context,
  ...
  // Upstream's dictionary-binding generator emits a getFooOr(fallback)
  // accessor for every optional member; shot's generator (tools/shot) does
  // not, so this reads through hasEventType()/eventType() instead. See
  // out_of_scope in the round1 report.
  if ((options->hasEventType() ? options->eventType() : String()).empty()) {
