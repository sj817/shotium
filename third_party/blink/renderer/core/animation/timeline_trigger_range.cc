...
TimelineTriggerRange::ComputeTriggerBoundaries(double current_offset,
  ...
  // Was `ExceptionState exception_state(nullptr)`: the nullptr was a
  // v8::Isolate* meaning "no script realm to throw into". ExceptionState no
  // longer takes an isolate (it records code+message locally, see
  // exception_state.h), so the default constructor is the direct equivalent.
  ExceptionState exception_state;
