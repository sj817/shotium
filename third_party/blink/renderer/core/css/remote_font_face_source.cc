...
void RemoteFontFaceSource::BeginLoadIfNeeded() {
  ...
  if (font->StillNeedsLoad()) {
    ...
    if (execution_context->Fetcher()->StartLoad(font)) {
      ...
      // core/lcp_critical_path_predictor's browser-fed LCP-image/font
      // prediction (frame->GetLCPP()->OnFontFetched()) is out of this cut's
      // reach: it's a cross-navigation ML heuristic reported over mojo, not
      // rendering logic, and core/frame/local_frame.h still only forward-
      // declares LCPCriticalPathPredictor with no header behind it. Dropped
      // here; local_frame.h's own dangling GetLCPP()/lcpp_ are core/frame's
      // call, not core/css's.
    }
  }
