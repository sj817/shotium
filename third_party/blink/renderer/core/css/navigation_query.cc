...
const URLPattern* NavigationLocation::FindOrCreateURLPattern(
  ...
  if (type_ == kUrlPattern || type_ == kUrl) {
    ...
    //
    // Was URLPattern::Create(isolate, MakeGarbageCollected<V8URLPatternInput>(
    // value_), document.Url(), IGNORE_EXCEPTION). `value_` is a plain String
    // and always was; the union it was wrapped in existed so that the WebIDL
    // constructor could accept either a string or a URLPatternInit, and
    // Create() unwrapped it again on the first line. The isolate was forwarded
    // to V8's RegExp engine, which is gone. See url_pattern.h.
    URLPattern* pattern =
        URLPattern::Create(value_, document.Url(), IGNORE_EXCEPTION);
    return pattern;
  }
