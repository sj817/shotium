...
void StyleRuleLocation::CreateRouteIfNeeded(Document* document) const {
  ...
  if (pattern_) {
    ...
    // str), document->Url(), IGNORE_EXCEPTION). The isolate only reached V8's
    // RegExp engine and the union only wrapped `str` for Create() to unwrap
    // again; both are gone. See url_pattern.h.
    url_pattern = URLPattern::Create(str, document->Url(), IGNORE_EXCEPTION);
  } else {
    ...
    // Likewise, the other arm of the union: the init goes straight in.
    url_pattern = URLPattern::Create(init, IGNORE_EXCEPTION);
  }
