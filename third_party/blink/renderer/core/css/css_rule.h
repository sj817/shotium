...
class CORE_EXPORT CSSRule : public ScriptWrappable {
 ...
 private:
  // VerifyParentIsCSSRule()/VerifyParentIsCSSStyleSheet() used to double-check
  // the reinterpret_casts below via V8's WrapperTypeInfo RTTI
  // (ToWrapperTypeInfo(parent_)->IsSubclass(...)). WrapperTypeInfo no longer
  // exists (see script_wrappable.h) and CSSRule/CSSStyleSheet share no cppgc
  // base that could reconstitute an equivalent IsA<>() check, so the extra
  // verification is gone; parent_is_rule_ below remains the real invariant.
