...
bool CSSDefaultStyleSheets::EnsureDefaultStyleSheetsForElement(
  ...
  // The <permission>-family element base (HTMLCapabilityElementBase) and its
  // subclasses (<install>, <login>, <credential>, <usermedia>, <camera>,
  // <microphone>) were removed entirely: they are browser permission-prompt
  // UI, and a screenshot engine never surfaces permission prompts. So
  // `permission_element_style_sheet_` is never populated any more; it stays
  // declared (see the header) purely so the Reset()/Trace()/
  // ForEachRuleFeatureSet() bookkeeping below doesn't need touching.
