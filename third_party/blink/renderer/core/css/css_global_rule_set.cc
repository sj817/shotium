...
void CSSGlobalRuleSet::UpdateDocumentRulesSelectorsRuleSet(Document& document) {
  MarkDirty();
  // The rule set was built from DocumentSpeculationRules::From(document)
  // .selectors(): the <a> selectors of the page's speculation *document rules*.
  // Its only purpose is to make style invalidation notice when a link starts or
  // stops matching one of those selectors, so the speculation machinery can
  // re-evaluate its candidates. core/speculation_rules is cut, nothing
  // contributes selectors, and the upstream code below already left the rule
  // set null in exactly that case (`document_rules_selectors.empty()`). So this
  // is the empty-input path, not a disabled one.
