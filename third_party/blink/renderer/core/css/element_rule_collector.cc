...
// Used to be IdentifiersFactory::IdForCSSStyleSheet(), which handed back the
// DevTools-protocol style sheet id (a WeakIdentifierMap entry keyed by
// InspectedFrames). core/inspector's IdentifiersFactory is gone with the rest
// of DevTools, so this just needs *a* stable per-CSSStyleSheet grouping key
// for the trace event below, not a DevTools-facing id: the sheet's own
// pointer identity does that.
String StyleSheetIdForPerfStats(const CSSStyleSheet* style_sheet) {
  if (!style_sheet) {
    return String();
  }
  return String::Number(reinterpret_cast<uintptr_t>(style_sheet));
}
...
void AggregateRulePerfData(
  ...
  for (const auto& rule_stats : rules_statistics) {
    ...
    CumulativeRulePerfKey key{rule_stats.selector_text,
                              StyleSheetIdForPerfStats(style_sheet)};
