...
bool SelectorChecker::CheckPseudoClass(const SelectorCheckingContext& context,
  ...
  switch (selector.GetPseudoType()) {
    ...
    case CSSSelector::kPseudoPermissionGranted:
      // HTMLCapabilityElementBase and every element it backed
      // (<install>, <login>, <credential>, <usermedia>, <camera>,
      // <microphone>) were removed entirely: they are browser
      // permission-prompt UI, and a screenshot engine never surfaces
      // permission prompts. No element can ever match this pseudo-class
      // any more.
      return false;
    case CSSSelector::kPseudoPictureInPicture:
      // No element is ever in picture-in-picture here; see
      // TreeScope::pictureInPictureElement(). The pseudo-class still parses,
