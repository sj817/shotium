...
base::expected<URLPattern*, String> ParseURLPatternFromJSON(
  ...
  if (String raw_string; pattern_value.AsString(&raw_string)) {
    // Upstream wrapped `raw_string` in a MakeGarbageCollected<V8URLPatternInput>
    // so that URLPattern::Create could immediately unwrap it again.  The string
    // overload takes it directly.
    return URLPattern::Create(raw_string, base_url, exception_state);
  }
  ...
  if (const JSONObject* pattern_object = JSONObject::Cast(&pattern_value)) {
    ...
    // URLPattern(input, baseURL) constructor steps given init.  Same unwrapping
    // as above, for the other arm of the union.
    return URLPattern::Create(init, exception_state);
  }
  return base::unexpected(
