...
// ScriptRegexp went with V8; url_pattern_regexp.h is the ICU-backed
// replacement. See that header for what changes and what does not.
...
class Component final : public GarbageCollected<Component> {
 public:
  ...
  // component string.
  // The `v8::Isolate*` upstream's first parameter carried existed only to
  // construct a ScriptRegexp, which needed an isolate to get at V8's RegExp
  // engine. ICU needs no such context, so the parameter is gone.
 ...
 private:
  ...
  // The pattern compiled down to a regular expression.  This is only
  // generated if `pattern_.CanDirectMatch()` returns false.  It used to be a
  // Member<ScriptRegexp>, i.e. garbage collected, because a ScriptRegexp held
  // a v8::RegExp the heap had to trace.  An ICU pattern is an ordinary heap
  // object owned outright by this Component.
  const std::unique_ptr<URLPatternRegexp> regexp_;
