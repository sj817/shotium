...
// v8_url_pattern_options.h is still where URLPatternOptions lives; it is now
// produced by tools/shot/gen_idl_dictionaries.py instead of by the bindings
// generator, from the same url_pattern_options.idl.
...
Component* Component::Compile(StringView pattern,
  ...
  std::unique_ptr<URLPatternRegexp> regexp;
  ...
  if (!parse_result.value().CanDirectMatch()) {
    ...
    // Compile the regular expression to verify it is valid.  Upstream built a
    // ScriptRegexp here, passing an isolate, kTextCaseSensitive/
    // kTextCaseAsciiInsensitive, MultilineMode::kMultilineDisabled and
    // UnicodeMode::kUnicodeSets.  URLPatternRegexp is compiled by ICU, needs
    // no isolate, and is never multiline; `options.sensitive` carries over as
    // its one flag.  The `v` flag has no ICU equivalent -- see
    // url_pattern_regexp.h -- and the effect of losing it is that a custom
    // regexp group using set notation now takes the "invalid group" path
    // below instead of compiling.
    DCHECK(base::IsStringASCII(regexp_string));
    regexp = std::make_unique<URLPatternRegexp>(String(regexp_string),
                                                options.sensitive);
    ...
    if (!regexp->IsValid()) {
      ...
      for (auto& part : parse_result.value().PartList()) {
        ...
        regexp =
            std::make_unique<URLPatternRegexp>(group_value, options.sensitive);
        if (regexp->IsValid())
          continue;
        exception_state.ThrowTypeError(
            StrCat({"Invalid ", TypeToString(type), " pattern '", final_pattern,
...
bool Component::Match(StringView input,
  ...
  if (regexp_) {
    ...
    // Upstream asked ScriptRegexp for the offset of the match and compared it
    // against 0.  URLPatternRegexp answers the anchored question directly.
    bool result = regexp_->Match(input, group_list ? &value_list : nullptr);
...
void Component::Trace(Visitor*) const {
  // Nothing to trace any more: `regexp_` was the one garbage-collected field,
  // and only because a ScriptRegexp owned a v8::RegExp.  The method stays
  // because URLPattern holds Members of us and the heap still has to walk in.
}
