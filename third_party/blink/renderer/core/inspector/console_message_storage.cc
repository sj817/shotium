...
bool ConsoleMessageStorage::AddConsoleMessage(ExecutionContext* context,
  ...
  // probe::ConsoleMessageAdded(context, message) used to notify an attached
  // DevTools frontend's console agent here. The protocol session handlers
  // and the probe itself are gone with them; `context` is otherwise unused
  // by this function and kept only for call-site compatibility.
