...
class CORE_EXPORT ConsoleMessage final
 ...
 public:
  ...
  // Two more constructors were here: one for a message coming out of a worker
  // (it tagged the message with the worker's DevTools token) and one built from
  // a WebConsoleMessage handed in by the embedder through the public API.
  // Workers are cut, and so is the embedder-facing WebConsoleMessage.
