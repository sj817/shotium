...
bool IsNavigationEventType(const Event& event) {
  ...
  // The Navigation API (NavigateEvent) was removed along with the rest of
  // the V8 bindings it required, so there is no longer a navigate_event
  // branch to check here.
