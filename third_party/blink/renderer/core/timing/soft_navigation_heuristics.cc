...
// ToV8NavigationType()/ToNavigationTypeForNavigationApi() used to come from
// core/navigation_api/navigation_type_util.h. That header, and the rest of
// the Navigation API implementation, was deleted along with V8 (only its
// .idl files survive; see also the NavigateEvent removal in
// event_timing.cc). These two conversions are pure enum-to-enum mappings
// with no V8 or navigation_api dependency, and soft-navigation heuristics
// still need to report a V8NavigationType/NavigationTypeForNavigationApi
// value, so they are reproduced locally here instead of reviving that
// directory.
...
constexpr mojom::NavigationTypeForNavigationApi
ToNavigationTypeForNavigationApi(V8NavigationType::Enum type) {
  switch (type) {
    case V8NavigationType::Enum::kPush:
      return mojom::NavigationTypeForNavigationApi::kPush;
    case V8NavigationType::Enum::kTraverse:
      return mojom::NavigationTypeForNavigationApi::kTraverse;
    case V8NavigationType::Enum::kReload:
      return mojom::NavigationTypeForNavigationApi::kReload;
    case V8NavigationType::Enum::kReplace:
      return mojom::NavigationTypeForNavigationApi::kReplace;
  }
  NOTREACHED();
}
