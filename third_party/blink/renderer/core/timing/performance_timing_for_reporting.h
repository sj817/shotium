...
class CORE_EXPORT PerformanceTimingForReporting final
 ...
 public:
  ...
  // GetNavigationTracingData() used to live here, producing a TracedValue
  // "navigationId" field (via the now-deleted IdentifiersFactory) for
  // DevTools tracing consumers. It had no caller left in the tree, so it was
  // deleted along with the rest of DevTools instrumentation.
