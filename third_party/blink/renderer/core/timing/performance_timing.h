...
class CORE_EXPORT PerformanceTiming final : public ScriptWrappable,
 ...
 public:
  ...
  // WriteInto(perfetto::TracedDictionary&) used to live here, writing a
  // "navigationId" field (via the now-deleted IdentifiersFactory) for
  // DevTools tracing consumers. It had no caller left in the tree -- nothing
  // feeds a PerformanceTiming through a TRACE_EVENT argument any more -- so
  // it was deleted along with the rest of DevTools instrumentation.
