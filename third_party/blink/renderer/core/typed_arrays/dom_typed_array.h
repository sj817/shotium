...
// DOMTypedArray's second template parameter used to be the matching V8 type
// (V8's Uint8Array, ...), which the class used only to pick the right
// `V8TypedArray::New()` when building a JS wrapper. There are no wrappers
// left, but the parameter still has to keep DOMUint8Array and
// DOMUint8ClampedArray distinct types (they share `uint8_t`), so it is now an
// incomplete Blink-local tag type per view kind.
#define DOMTYPEDARRAY_DECLARE_TAG(val_t, Type, clamped) struct Type##ArrayTag;
DOMTYPEDARRAY_FOREACH_VIEW_TYPE(DOMTYPEDARRAY_DECLARE_TAG)
#undef DOMTYPEDARRAY_DECLARE_TAG

template <typename T, typename TypeTag, bool clamped = false>
class DOMTypedArray final : public DOMArrayBufferView {
  typedef DOMTypedArray<T, TypeTag, clamped> ThisType;
...
#define DOMTYPEDARRAY_DEFINE_GETTYPE(val_t, Type, clamped)            \
  ...
  DOMTypedArray<val_t, Type##ArrayTag, clamped>::GetType() const {    \
...
#define DOMTYPEDARRAY_DEFINE_TYPEDEFNAME(val_t, Type, clamped) \
  using DOM##Type##Array = DOMTypedArray<val_t, Type##ArrayTag, clamped>;
