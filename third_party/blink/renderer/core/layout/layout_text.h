...
class CORE_EXPORT LayoutText : public LayoutObject {
 public:
  ...
  // FirstAbstractInlineTextBox()/HasAbstractInlineTextBox()/
  // SetHasAbstractInlineTextBox() were here: AbstractInlineTextBox existed
  // solely to serve the accessibility tree and is gone with it.
...
inline void LayoutText::DetachAxHooksIfNeeded() {
  NOT_DESTROYED();
  // Used to also detach this object's |AbstractInlineTextBox| here; that
  // class existed solely to serve the accessibility tree and is gone with it
  // (see core/layout/inline/abstract_inline_text_box.h's deletion note in
  // core/layout/build.gni).
  if (!IsInLayoutNGInlineFormattingContext()) {
    return;
  }
