...
const std::optional<PhysicalSize> LayoutEmbeddedContent::FrozenFrameSize()
    const {
  NOT_DESTROYED();
  // Only the `<fencedframe>` element (removed along with
  // core/html/fenced_frame/) could freeze the child frame size when
  // navigated; no other embedded content ever does.
  return std::nullopt;
}
...
void LayoutEmbeddedContent::PropagateZoomFactor(double zoom_factor) {
  NOT_DESTROYED();
  // This used to be skipped for `<fencedframe>` elements (removed along with
  // core/html/fenced_frame/), which opted out of embedder zoom propagation.
