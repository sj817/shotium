...
LayoutObject* LayoutObject::CreateObject(Element* element,
  ...
  switch (style.Display()) {
    ...
    case EDisplay::kInlineLayoutCustom:
      // "display: layout(name)" (the CSS Layout API) needs a layout worklet,
      // i.e. a script engine, to ever have a registered layout definition.
      // Without one no definition can exist, so these boxes permanently take
      // the "unloaded" fallback that LayoutCustom implemented: plain
      // block-flow layout. LayoutBlockFlow::CreatesNewFormattingContext()
      // keeps them formatting-context roots via IsDisplayLayoutCustom().
      return CreateBlockFlowOrListItem(element, style);
  }
...
void LayoutObject::StyleDidChange(
  ...
  // diff.ax_visibility_or_inert_changed / diff.ax_style_changed used to
...
void LayoutObject::ImageNotifyFinished(ImageResourceContent* image) {
  NOT_DESTROYED();
  // Used to notify the AXObjectCache that this object's image finished
  // loading; no accessibility tree exists to notify anymore.

  if (GetDocument().domWindow()) {
    PaintTimingDetector::From(GetDocument()).NotifyImageFinished(*this, image);
  }

  // A finished image used to propagate its AdProvenance onto the element and
  // count kAdImageHDR for HDR ads. A resource carries no provenance now that
  // neither AdTracker nor the subresource filter can annotate a request.
}
