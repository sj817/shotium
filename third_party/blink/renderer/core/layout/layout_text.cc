...
void LayoutText::StyleDidChange(
  ...
  // diff.ax_visibility_or_inert_changed used to notify the AXObjectCache
  // that this object's visibility/inertness changed; no accessibility tree
  // exists to notify anymore.
...
void LayoutText::ClearBlockFlowCachedData() {
  NOT_DESTROYED();
  // Used to notify the AXObjectCache to clear its cached fragment data and
  // that inline text boxes were updated; no accessibility tree exists to
  // notify anymore.
}
...
void LayoutText::SetFirstInlineFragmentItemIndex(wtf_size_t index) {
  ...
  DCHECK_NE(index, 0u);
  DetachAxHooksIfNeeded();
  // Used to notify the AXObjectCache that this LayoutText's text child
  // might need to be recomputed; no accessibility tree exists to notify
  // anymore.
...
void LayoutText::TextDidChangeWithoutInvalidation() {
  ...
  // Used to notify the AXObjectCache that this text changed; no
...
// LayoutText::FirstAbstractInlineTextBox() was here: AbstractInlineTextBox
// existed solely to serve the accessibility tree and is gone with it.
...
DOMNodeId LayoutText::EnsureNodeId() {
  NOT_DESTROYED();
  if (node_id_ == kInvalidDOMNodeId) {
    // ContentCaptureManager::ScheduleTaskIfNeeded() was called here before
    // the id was minted; content capture is cut, so the id is minted directly.
