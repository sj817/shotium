...
// GetMediaStreamDescriptor() and GetMediaSourceHandle() were here. Both read
// HTMLMediaElement::GetSrcObjectVariant(), a std::variant over
// MediaStreamDescriptor* (platform/mediastream, cut with the rest of WebRTC)
// and MediaSourceHandle* (core/html/media, cut with the rest of Media Source
// Extensions -- there is no demux/decode pipeline left for it to hand a
// SourceBuffer to). With both variant members gone, srcObject itself no
// longer has anything to hold, so HTMLMediaElement dropped
// SetSrcObjectVariant()/GetSrcObjectVariant() and these two accessors have
