...
void AnchorEvaluatorImpl::UpdateAccessibilityAnchor(
    const LayoutObject* anchor) {
  // This tracking only mattered for exposing the CSS anchor-position anchor
  // to the accessibility tree via AXObjectCache; no accessibility tree
  // exists to expose it to anymore, so tracking is skipped unconditionally
  // (this early return used to be gated on an active AXObjectCache).
}
