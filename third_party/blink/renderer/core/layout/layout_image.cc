...
void GetImageSizeChangeTracingData(perfetto::TracedValue context,
  ...
  // Used to be IdentifiersFactory::IntIdForNode()/FrameId(), which are gone
  // with the rest of core/inspector. node->GetDomNodeId() is the same value
  // IntIdForNode() returned; GetFrameIdForTracing() (core/frame/frame.h) is
  // the same generic tracing helper FrameId() forwarded to.
  dict.Add("nodeId", node->GetDomNodeId());
  dict.Add("frameId", GetFrameIdForTracing(frame));
}
...
bool LayoutImage::ForegroundIsKnownToBeOpaqueInRect(
  ...
  // DEVTOOLS_TIMELINE_TRACE_EVENT_WITH_CATEGORIES(...) was here.
