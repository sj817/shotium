...
bool XMLDocumentParser::ParseDocumentFragment(
  ...
  // Used to be a TryRethrowScope: if parsing the fragment ran script (e.g.
  // a custom element constructor) that threw, the V8 exception was rethrown
  // through `exception_state` so it would propagate to the caller's JS
  // context. There is no V8 and no script execution anymore, so nothing can
  // throw here for `exception_state` to catch and rethrow.
...
void XMLDocumentParser::StartElementNs(
  ...
  if (!parsing_fragment_) {
    if (HTMLConstructionSite::LookUpCustomElementDefinition(*document_, q_name,
      ...
      // CEReactionsScope used to take a v8::Isolate* to run queued custom
      // element reactions against; it no longer needs one now that reactions
      // can't touch V8 (there is no V8).
      reactions.emplace();
    }
  }
...
void XMLDocumentParser::DoEnd() {
  ...
  if (xml_viewer_mode) {
    // The pretty XML tree view was built by running
    // documentxmltreeviewer.js in an isolated world. With no script engine
    // there is nothing to run it in, so a style-less XML document is left to
    // render through the default XML rendering rules instead.
