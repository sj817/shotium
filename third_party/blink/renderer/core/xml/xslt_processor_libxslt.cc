...
void XSLTProcessor::ParseErrorFunc(void* user_data, const xmlError* error) {
  ...
  // SourceLocation used to take a trailing V8 stack trace (captured from the
  // JS call stack active when the XSLT error fired); it no longer accepts
  // one now that there's no V8 call stack to capture.
