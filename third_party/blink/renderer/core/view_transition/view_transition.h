...
// Upstream got this declaration transitively, from the generated
// v8_sync_iterator_view_transition_type_set.h -- the setlike iterator for the
// IDL interface. Nothing generates sync iterators here (they exist only to be
// stepped from script), and the type is used below only as Member<> and a
// returned pointer, so a forward declaration is all that is needed.
