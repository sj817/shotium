...
class PageSwapEvent final : public Event {
 ...
 public:
  PageSwapEvent(Document&, mojom::blink::PageSwapEventParamsPtr);
  ...
  // activation() was here, returning the NavigationActivation that described
  // which history entry the page was swapping to. core/navigation_api is cut,
  // so the event carries no payload -- see page_swap_event.cc.
};
