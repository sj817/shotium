...
void ViewTransition::ProcessCurrentState() {
  ...
  while (process_next_state) {
    ...
    switch (state_) {
      ...
      case State::kCaptureCommitted:
      case State::kCaptured: {
        // The DOM update callback used to run here for script-initiated
        // transitions, optionally as early as capture-commit. With no script
        // transitions left, only the kCaptured phase does anything.
...
void ViewTransition::ActivateFromSnapshot() {
  ...
  // This used to force the V8 microtask queue to drain here so the
  // ViewTransition promises (exposed to script) were settled before the next
  // rendering steps, since in the cross-document case activation isn't
  // called from inside a script. See
  // https://github.com/whatwg/html/pull/10284
  // There is no V8 and no script in this process, so there are no promises
  // to settle and no microtask queue to drain; the scope is gone.
