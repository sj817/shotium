...
void ViewTransitionSupplement::StartTransition(
  ...
  auto* page_swap_event =
      MakeGarbageCollected<PageSwapEvent>(document, std::move(params));
...
bool ViewTransitionSupplement::HasNonScriptTransitions() const {
  // document.startViewTransition() is gone, so every transition that can
  // exist here is navigation-initiated, i.e. non-script.
  return document_transition_ || !element_transitions_.empty();
}

bool ViewTransitionSupplement::IsEarlyCallbackEnabled() const {
  // Running the author's DOM update callback as early as capture-commit only
  // ever applied to script-initiated transitions, of which there are none.
  return false;
}
...
void ViewTransitionSupplement::ResolveCrossDocumentViewTransition() {
  if (!document_transition_ ||
      !document_transition_->IsForNavigationOnNewDocument()) {
    return;
  }
