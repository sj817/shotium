...
void SVGSVGElement::ParseAttribute(const AttributeModificationParams& params) {
  ...
  // `onunload`, `onresize`, `onscroll` (window listeners installed by the
  // outermost <svg> element) and `onabort`/`onerror` (window listeners
  // installed by any <svg>) used to compile their content attribute value
  // into a callable EventListener via JSEventHandlerForContentAttribute,
  // which required V8 to run the resulting script. There is no script
  // engine in this renderer, so those content-attribute event handlers can
  // never fire and are no longer wired up here.
