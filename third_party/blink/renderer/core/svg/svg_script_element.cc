...
void SVGScriptElement::ParseAttribute(
  ...
  // `onerror` used to compile the content attribute value into a callable
  // EventListener via JSEventHandlerForContentAttribute, which required V8
  // to run the resulting script. There is no script engine in this
  // renderer, so it can never fire and is no longer wired up here.
  if (params.name == svg_names::kAsyncAttr &&
