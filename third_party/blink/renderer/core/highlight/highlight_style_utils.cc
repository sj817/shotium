...
Color DefaultBackgroundColor(
  ...
  switch (pseudo) {
    ...
    case kPseudoIdTargetText:
      ...
      // kFragmentTextBackgroundColorARGB (components/shared_highlighting),
      // but that component was deleted wholesale along with the
      // browser-side text-fragment "copy link to highlight" feature it
      // served. The constant itself (0xFFE9D2FD, a pale purple) is just a
      // color value with no dependency on the deleted machinery, so it is
      // inlined here.
      return Color::FromRGBA32(0xFFE9D2FD);
    case kPseudoIdSpellingError:
    case kPseudoIdGrammarError:
    case kPseudoIdHighlight:
...
Color HighlightStyleUtils::ResolveColor(
  ...
  std::optional<Color> maybe_color = MaybeResolveColor(
      document, originating_style, pseudo_style, pseudo, property, search_text_is_active_match);
  if (maybe_color) {
...
HighlightStyleUtils::HighlightPaintingStyle(
  ...
  if (!uses_text_as_clip && !ignored_selection) {
    ...
    maybe_color = MaybeResolveColor(document, originating_style, pseudo_style,
                                    pseudo, GetCSSPropertyWebkitTextFillColor(), search_text_is_active_match);
    if (maybe_color) {
    ...
    maybe_color = MaybeResolveColor(document, originating_style, pseudo_style,
                                    pseudo, GetCSSPropertyTextEmphasisColor(), search_text_is_active_match);
    if (maybe_color) {
    ...
    maybe_color = MaybeResolveColor(document, originating_style, pseudo_style,
                                    pseudo, GetCSSPropertyTextDecorationColor(), search_text_is_active_match);
    if (maybe_color) {
