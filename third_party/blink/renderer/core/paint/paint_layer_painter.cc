 ...
 public:
  PaintTimelineReporter(const PaintLayer& layer, bool should_paint_content)
    ...
    if (ShouldReport(should_paint_content)) {
      ...
      // The "data" payload -- the layer's frame, layout object and cull rect,
      // written by inspector_paint_event::Data -- is gone with core/inspector's
      // trace-event data writers. The Paint slice itself is kept: it is how the
      // paint phase shows up in a trace, and its begin/end pairing is what
      // PaintTimelineReporter exists to maintain.
      TRACE_EVENT_BEGIN(kDevToolsTimelineCategory, "Paint");
    }
  }
...
PaintResult PaintLayerPainter::Paint(GraphicsContext& context,
  ...
  if (should_paint_content) {
    ...
    bool ensure_chunk = properties && properties->Filter() &&
