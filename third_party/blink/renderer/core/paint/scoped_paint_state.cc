...
void ScopedBoxContentsPaintState::AdjustForBoxContents(const LayoutBox& box) {
  ...
  // A MobileFriendlinessChecker block was here: for a horizontally scrollable
  // box it reported the painted content rect as "replaced" content and then
  // suppressed beyond-viewport accounting for that box and for position:fixed
  // boxes. All of it fed a UKM score; core/mobile_metrics is cut.
}
