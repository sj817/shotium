...
// Used to be core/inspector/identifiers_factory.cc's
// IdentifiersFactory::LoaderId(), which is gone with the rest of DevTools'
// core/inspector. Reimplemented directly (not restored) since it's one line
// and DocumentLoader::GetDevToolsNavigationToken() -- the underlying data --
// is still there; this trace value is for chrome://tracing, not the
// (deleted) inspector protocol.
String LoaderIdForTracing(DocumentLoader* loader) {
  if (!loader) {
    return String();
  }
  return String(loader->GetDevToolsNavigationToken().ToString());
}
...
LargestContentfulPaintCalculator::CreateWebExposedCandidateTraceDataCommon(
  ...
  if (delegate_->IsHardNavigation()) {
    value->SetString("navigationId",
                     LoaderIdForTracing(window->document()->Loader()));
  }
  value->SetInteger(
