...
class CORE_EXPORT OriginTrialContext final
 ...
 private:
  ...
  // Applies the side effects of a series of OriginTrialFeatures listed in a
  // HashSet: sending the trial tokens to the browser process and honouring
  // per-feature opt-outs. This used to additionally install the features'
  // JavaScript bindings on the main-world context, which is where its
  // script-context argument and bool return came from; there are no bindings
  // install any more.
  void InstallFeatures(const HashSet<mojom::blink::OriginTrialFeature>& features,
                       Document&);
