...
void OriginTrialContext::InitializePendingFeatures() {
  ...
  InstallFeatures(enabled_features_, *window->document());
  InstallFeatures(navigation_activated_features_, *window->document());
}

void OriginTrialContext::InstallFeatures(
  ...
  for (mojom::blink::OriginTrialFeature enabled_feature : features) {
    ...
    if (enabled_feature ==
      ...
      // The use counter still fires; only the report to the browser's
      // performance manager is gone with DocumentResourceCoordinator. That
      // report told the browser not to freeze this browsing context group,
      // which matters to a browser that freezes documents. This one does not.
...
const SecurityOrigin* OriginTrialContext::GetSecurityOrigin() {
  CHECK(context_);
  // Used to special-case WorkletGlobalScope here: for the purpose of origin
  // trials, worklets are considered to run in the same context as their
  // originating document, so the origin was read off that document instead
  // of the worklet itself. There are no worklets anymore (no workers at
  // all), so context_ is always the plain execution context (a document's
  // window) and its own origin is always the right answer.
  return context_->GetSecurityOrigin();
}

bool OriginTrialContext::IsSecureContext() {
  CHECK(context_);
  // Used to special-case WorkletGlobalScope here: worklets are spec'd to
  // never be secure contexts on their own (unique origin), so origin trials
  // instead checked the secure-context status of the originating document.
  // There are no worklets anymore (no workers at all), so context_ is
  // always the plain execution context and its own status is always the
  // right answer.
  return context_->IsSecureContext();
}
