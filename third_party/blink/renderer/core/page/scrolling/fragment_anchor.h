...
class CORE_EXPORT FragmentAnchor : public GarbageCollected<FragmentAnchor> {
 public:
  ...
  // IsTextFragmentAnchor() let callers tell a TextFragmentAnchor (":~:text="
  // links) apart from an ElementFragmentAnchor ("#id" links) without an RTTI
  // cast. TextFragmentAnchor is deleted -- ElementFragmentAnchor is the only
  // subclass left -- and nothing checked this, so the distinction is gone too.
