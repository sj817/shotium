...
class CORE_EXPORT CachedPermissionStatus final
 ...
 private:
  // The only Client this cache ever had (HTMLCapabilityElementBase, the
  // <permission>-family element base class) was removed along with the rest
  // of that element family -- see the html_capability_element_base.* removal.
  // A screenshot engine never surfaces permission prompts, so there is no
  // in-tree caller left to register as a Client; this class is kept wired to
  // DocumentLoader (see the `friend` below) but is otherwise unused.
