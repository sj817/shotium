...
class CORE_EXPORT RemoteFrameView final
 ...
 public:
  ...
  // Print() removed in this cut: it built a placeholder content ID via
  // printing::MetafileSkia::CreateContentForRemoteFrame() and told the
  // browser to print the cross-process subframe over
  // GetRemoteFrameHostRemote().PrintCrossProcessSubframe(). printing/ has
  // been stripped to its mojom (no MetafileSkia implementation remains), and
  // grepping the tree found no caller left for this method either -- a
  // screenshot engine never prints.
  // CapturePaintPreview() removed in this cut -- see the comment at its
  // former call site in Paint() (remote_frame_view.cc).
