...
void ExecutionContextCSPDelegate::SetSandboxFlags(
  ...
  // Workers and Worklets don't share agents (each one is unique), so they
  // used to be allowed to apply new sandbox flags on top of the current
  // ones here via WorkerOrWorkletGlobalScope::SetSandboxFlags. Workers
  // require a script engine to run, which this renderer no longer has, so
  // WorkerOrWorkletGlobalScope is gone and execution_context_ is always a
  // (non-worker) Document's LocalDOMWindow.
...
SourceLocation* ExecutionContextCSPDelegate::GetSourceLocation() {
  ...
  // location, same as this call would degrade to upstream with no active
  // script.
...
void ExecutionContextCSPDelegate::PostViolationReport(
  ...
  //
  // This used to check DynamicTo<WorkletGlobalScope>(...); WorkletGlobalScope
  // required a script engine to run and is gone along with it, but
  // ExecutionContext::IsWorkletGlobalScope() is kept (always false now) as
  // the general "is this a worklet" query, so it still reads correctly here.
...
void ExecutionContextCSPDelegate::DispatchViolationEventInternal(
  ...
  // This used to fall back to dispatching on a WorkerGlobalScope when there
  // was no Document. WorkerGlobalScope required a script engine to run and
  // is gone along with it; LocalDOMWindow is the only ExecutionContext left,
  // so GetDocument() below is expected to always succeed.
