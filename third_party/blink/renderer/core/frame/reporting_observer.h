...
class CORE_EXPORT ReportingObserver final
 ...
 public:
  ...
  explicit ReportingObserver(ExecutionContext*, ReportingObserverOptions*);

  // ActiveScriptWrappable
  bool HasPendingActivity() const final;

  // Drains |report_queue_|. Upstream this invoked the script callback with
  // the drained reports; there is no callback, so the reports are discarded.
  // takeRecords() below still returns anything queued since the last drain.
