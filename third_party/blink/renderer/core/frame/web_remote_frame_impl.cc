...
void WebRemoteFrameImpl::InitializeCoreFrame(
  ...
  //
  // The `owner && owner->IsLocal()` branch that used to live here only ever
  // triggered for a RemoteFrame owned locally by a <fencedframe> element
  // (which is not part of the regular frame tree, hence traversing up through
  // FrameOwner instead of `parent`). Fenced frames have been removed from
  // this engine along with their sole creation path
  // (WebLocalFrameImpl::CreateFencedFrame), so that case can no longer occur.
