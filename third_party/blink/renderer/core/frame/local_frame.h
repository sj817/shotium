...
class CORE_EXPORT LocalFrame final
 ...
 public:
  ...
  // Note: these functions are not virtual but intentionally shadow the
  ...
  // browser process. A static screenshot engine has no cross-navigation
  // browser process feeding it heuristics, so there is nothing to predict
  // for. All callers below (web_local_frame_impl.cc, html_image_element.cc)
  // were updated to drop their now-dead LCPP branches.
  ...
  // GetInspectorTaskRunner() removed in this cut: it exposed
  // inspector_task_runner_ (InspectorTaskRunner, the devtools protocol
  // dispatch thread's task runner -- deleted along with the rest of the
  ...
  // GetScriptObserver()/LCPScriptObserver removed in this cut along with
  // LCPCriticalPathPredictor -- see the comment on the (now deleted)
  // GetLCPP() declaration site above.
  ...
  // frame was an ad were AdTracker, which walked the V8 stack, and the
  // subresource filter, whose only implementation was in
  // //components/subresource_filter. Both are gone, and no caller of
  // WebLocalFrame::SetAdEvidence() is left in the tree, so a frame's ad
  // evidence could only ever have stayed unset.
  ...
  // Its only caller, LocalFrameMojoHandler::PostMessageEvent(), now discards
  ...
  // TextFragmentHandler (core/dom's scroll-to-text-fragment machinery) was
  // deleted along with the file that defined it; nothing in this build
  // constructs one any more. document.cc still calls this to conditionally
  // notify a handler on detach, so the accessor stays -- "there is never a
  // text fragment handler" is now permanently true.
  TextFragmentHandler* GetTextFragmentHandler() const { return nullptr; }

  // BindTextFragmentReceiver() and CreateTextFragmentHandler() removed in
  // this cut: they wired the browser's mojom::blink::TextFragmentReceiver
  // interface (scroll-to-text-fragment) to a TextFragmentHandler, a class
  // that no longer exists (see GetTextFragmentHandler() above). The
  // AddInterface() registration for BindTextFragmentReceiver is gone from
  // the .cc too.
  ...
  // GetV8LocalCompileHintsProducer() removed in this cut: it exposed
  // v8_local_compile_hints_producer_ (V8LocalCompileHintsProducer, deleted
  // with V8) and had no caller left once MainFrameInteractive() /
  // MainFrameFirstMeaningfulPaint() (above) were removed.
 ...
 private:
  ...
  // v8_local_compile_hints_producer_, V8HistogramAccumulator -- all gone
  // with V8). FrameScheduler::Delegate's base implementations are already
  // empty (see frame_scheduler.h), so dropping the overrides changes
  // nothing observable.
  ...
  // inspector_task_runner_ removed in this cut -- see
  // GetInspectorTaskRunner() above.
  Member<PerformanceMonitor> performance_monitor_;

  Member<IdlenessDetector> idleness_detector_;
  base::OnceClosureList network_idle_callbacks_;
  // inspector_trace_events_ removed in this cut: InspectorTraceEvents
  // (devtools performance-trace event forwarding) no longer exists, and
  // CoreProbeSink lost the AddInspectorTraceEvents()/
  // RemoveInspectorTraceEvents() methods that would have registered it.
  ...
  // text_fragment_handler_ removed in this cut -- see
  // GetTextFragmentHandler() above.
  ...
  // v8_local_compile_hints_producer_ removed in this cut -- see
  // GetV8LocalCompileHintsProducer() above.
