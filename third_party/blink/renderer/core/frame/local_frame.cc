...
// SourceFrameForOptionalToken() removed in this cut: an unused helper
// (-Wunused-function) that resolved an optional RemoteFrameToken to a
// RemoteFrame*; its caller was already gone by the time this pass reached
// the file.
...
// DocumentReadyStateToString() removed in this cut: an unused helper
// (-Wunused-function) that rendered a DocumentReadyState as a debug string;
// its caller was already gone by the time this pass reached the file.
...
void LocalFrame::Init(
  ...
  // Used to register BindTextFragmentReceiver as the mojom::blink::
  // TextFragmentReceiver interface handler here; removed along with that
  // method -- see GetTextFragmentHandler()'s comment in the header.
...
void LocalFrame::Trace(Visitor* visitor) const {
  ...
  // inspector_issue_reporter_ traced InspectorIssueReporter, deleted with
  // the rest of the devtools issue-reporting machinery; no script engine
  // means no issues to report against. inspector_trace_events_ was traced
  // just below and is gone for the same reason (see the header).
...
bool LocalFrame::DetachImpl(FrameDetachType type) {
  ...
  // class (InspectorIssueReporter) are gone with devtools issue reporting.
  // Used to remove inspector_trace_events_ from probe_sink_ here, then
...
// LCPCriticalPathPredictor itself -- see the comment on the (now deleted)
// declaration site in local_frame.h.
...
void LocalFrame::HookBackForwardCacheEviction() {
  ...
  // The eviction-on-script-execution hook lived in the script engine; with no
  // script engine a frozen frame can never be mutated by script, so there is
...
void LocalFrame::SetIsInert(bool inert) {
  ...
  // Used to refresh the accessibility tree, since nodes can change inertness
  // which means they must be added or removed from it; no accessibility
  // tree exists to refresh anymore.
}
...
// LocalFrame::GetInspectorTaskRunner() removed in this cut -- see its
// former declaration in the header.
...
LocalFrame::LocalFrame(
    ...
    : Frame(client,
      ...
      // Used to init inspector_task_runner_ via InspectorTaskRunner::Create()
      // here; that class is gone -- see its former declaration in the
      // header.
      interface_registry_(interface_registry
                              ? interface_registry
                              : InterfaceRegistry::GetEmptyInterfaceRegistry()),
      // Used to init v8_local_compile_hints_producer_ here; gone with V8 --
      // see its former declaration in the header.
  ...
  if (IsLocalRoot()) {
    performance_monitor_ =
        MakeGarbageCollected<PerformanceMonitor>(this);

    // Used to create an InspectorTraceEvents and register it on
    // probe_sink_ here; that class (devtools performance-trace forwarding)
    // is gone -- see inspector_trace_events_'s former declaration in the
    // header -- and CoreProbeSink lost the Add/RemoveInspectorTraceEvents()
    // methods that would have taken it.
    // LCPScriptObserver removed in this cut along with
    // LCPCriticalPathPredictor (see GetLCPP()'s comment above): it fed
    // observed script URLs into the same deleted LCPP heuristic database.
  } else {
  ...
  // An AdTracker was created here on the local root when the AdTagging runtime
  // feature was on, and every frame then asked it whether an ad script was on
  // the V8 stack at creation time. It walked the JS stack; there is no stack.
...
// Content capture walked the on-screen text of a frame and streamed it to
// the embedder (WebContentCaptureClient) for Android's on-device page
// understanding. It is a reader of layout, not part of it.
...
// IsAdFrame(), IsAdRoot(), SetAdEvidence(), IsAdScriptInStack(),
// CreationAdScript() and UpdateAdHighlight() were here.
//
// SetAdEvidence() was the single way a frame ever became an ad: the embedder
// called WebLocalFrame::SetAdEvidence() from SubresourceFilterAgent, which
// lived in content/renderer and is gone along with //components/subresource_filter.
// No caller is left anywhere in the tree, so ad evidence could only stay unset
// and every one of these answers was fixed. What went with them: the frame
// scheduler's ad bit, the kAdSubframeCounter instance counter, the
// kAdFrameDetected use counter, Element::SetIsAdRelated() on the owner, and the
// red "highlight ads" overlay -- which needed a WebPreference this build never
// sets. IsAdScriptInStack() and CreationAdScript() answered from AdTracker,
// which walked the V8 stack.
...
// LocalFrame::MainFrameInteractive() and MainFrameFirstMeaningfulPaint()
// removed in this cut -- see the comment on their former declarations in
// the header.
...
void LocalFrame::ForciblyPurgeMemory() {
...
void LocalFrame::MediaPlayerActionAtViewportPoint(
  ...
  switch (type) {
    ...
    case mojom::blink::MediaPlayerActionType::kPictureInPicture:
      // PictureInPictureController removed in this cut: it lived under
      // deleted modules/, spoke to a deleted media_session mojom, and
      // manages an actual browser window for the video -- a screenshot
      // engine has no window to place the video into, so there is nothing
...
// PostMessageEvent() used to build a MessageEvent out of the incoming
// BlinkTransferableMessage (a structured clone of the script value passed to
// postMessage(), plus transferred ports and user-activation state) and
// dispatch it on this frame's DOMWindow. A dispatched MessageEvent can only
// ever be observed by a script 'message' event listener, and there is no
// script engine to run one, so LocalFrameMojoHandler::PostMessageEvent() (its
// only caller) now discards the incoming message directly instead of calling
// this.
