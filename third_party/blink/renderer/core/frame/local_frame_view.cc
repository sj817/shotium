...
LocalFrameView::LocalFrameView(LocalFrame& frame, gfx::Rect frame_rect)
    : FrameView(frame_rect),
      ...
      // mobile_friendliness_checker_ and tap_friendliness_checker_ used to be
      // constructed here. Both classes lived in core/mobile_metrics, a UKM
      // reporting subsystem deleted wholesale ("remove 33 blink core
      // subdirectories that the render pipeline never enters"); there is no
      // metrics reporting in a screenshot engine.
#if DCHECK_IS_ON()
      ,
...
void LocalFrameView::RunPostLifecycleSteps() {
  {
    ...
    // Used to poke mobile_friendliness_checker_->MaybeRecompute() here; the
    // mobile-friendliness UKM checker is gone (see the constructor).
...
bool LocalFrameView::NotifyResizeObservers() {
  ...
  if (min_depth != ResizeObserverController::kDepthBottom) {
    ...
    if (resize_controller->SkippedObservations() &&
      ...
      // CaptureSourceLocation() used to take frame_->DomWindow() and walk
      // the current V8 stack for a location; with no V8 stack to walk, the
      // no-arg overload (an always-unknown location) is what is left.
      // ErrorEvent::Create(message, location) also used to take a 3rd
      // argument; ErrorEvent's 2-arg factory (see error_event.h) no longer
      // takes one.
...
void LocalFrameView::RunPaintLifecyclePhase(PaintBenchmarkMode benchmark_mode) {
  ...
  ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) {
    if (auto* scrollable_area = frame_view.GetScrollableArea())
      scrollable_area->UpdateScrollAnimationState();
    for (PaintLayerScrollableArea* area : frame_view.animating_scrollable_areas_)
      area->UpdateScrollAnimationState();

    // Defer ready-promise callbacks until the lifecycle update is complete.
    ScriptForbiddenScope forbid_script;
    frame_view.GetLayoutView()->GetDocument().GetDocumentAnimations()
        .UpdateAnimations(DocumentLifecycle::kPaintClean, true);
  });
}

void LocalFrameView::RunAccessibilitySteps() {
  // Used to commit pending AXObjectCache updates, compute Android XR hit
  // test order, and serialize the accessibility tree for every
  // non-throttled frame view; no accessibility tree exists to step
  // anymore.
}
...
void LocalFrameView::PaintTree(
  ...
  // Used to open a MobileFriendlinessChecker::PaintScope around the paint
  // below; the checker class is gone (see the constructor).
...
// LocalFrameView::CapturePaintPreview() removed in this cut -- see the
// comment at its call site in Paint() below.
...
void LocalFrameView::Paint(const PaintInfo& paint_info,
  ...
  if (owner_layout_object &&
    ...
    // LocalFrameView::CapturePaintPreview() removed in this cut: paint
    // preview is "save the page as a replayable paint record for the
    // browser to composite later", driven by a browser-process mojo call
    // (components/paint_preview, deleted) that asked this scrollable
    // subframe to be captured out-of-band. A screenshot engine paints
    // straight to pixels and has no browser process to coordinate a
    // separate subframe capture with, so this fallback-to-inline-paint
    // path (falling through below) is always taken now.
  }
...
void LocalFrameView::RegisterTapEvent(Element* target) {
  // Used to forward to tap_friendliness_checker_->RegisterTapEvent(target);
  // TapFriendlinessChecker (core/mobile_metrics) is gone -- see the
  // constructor -- so there is no tap-friendliness UKM to record. The
  // function stays because core/input/gesture_manager.cc (outside this
