...
void RemoteFrame::Navigate(FrameLoadRequest& frame_request,
  ...
  // The Sandbox half of this policy still applies. The AdFrame half does not:
  // the initiator's ad status came from LocalFrame::IsAdFrame() and
  // IsAdScriptInStack(), and nothing in this build can tag a frame or a script
  // as an ad. The browser-side download policy and NavigationHandle::StartedByAd()
  // keep their parameters, so pass the answer that is true here.
  params->download_policy.ApplyDownloadFramePolicy(
      ...
      initiator_frame_has_download_sandbox_flag, /*from_ad_frame=*/false,
      /*is_ad_script_in_stack=*/false);

  params->started_by_ad = false;
...
// ForwardPostMessage() used to route a postMessage() call targeting a
// cross-process frame to the browser (RouteMessageEvent), carrying a
// BlinkTransferableMessage -- the structured clone of the script value
// passed to postMessage(), plus transferred ports. postMessage() has no
// caller without a script engine (see remote_dom_window.cc), and had no
// remaining caller here either, so it is gone along with
...
void RemoteFrame::SetReplicatedOrigin(
  ...
  // Used to notify the AXObjectCache that the owner element's accessibility
  // object now points to a different child, because the origin of this
  // remote frame changed; no accessibility tree exists to notify anymore.
}

// IsAdFrame() and SetReplicatedIsAdFrame() were here, along with the replicated
// `is_ad_frame_` bit. The browser only ever set that bit from
// RenderFrameHostImpl::UpdateIsAdFrame(), whose sole non-test caller was
// //components/subresource_filter's throttle manager, deleted. Its one effect
// in the renderer was marking the owner element ad-related for the "highlight
// ads" overlay, which is gone too.
