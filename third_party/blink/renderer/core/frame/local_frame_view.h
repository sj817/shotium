...
class CORE_EXPORT LocalFrameView final
 ...
 public:
  ...
  // GetMobileFriendlinessChecker() removed in this cut: it exposed
  // mobile_friendliness_checker_, a UKM-reporting class deleted along with
 ...
 private:
  ...
  // CapturePaintPreview() removed in this cut: it sent an IPC asking the
  // (now-deleted) browser process to record this frame as a separate paint
  // preview SkPicture. See the comment at its former call site in
  // Paint() (local_frame_view.cc).
  ...
  // mobile_friendliness_checker_ and tap_friendliness_checker_ removed in
  // this cut -- see the constructor in the .cc for why.
