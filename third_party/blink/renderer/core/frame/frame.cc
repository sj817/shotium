...
void Frame::NotifyUserActivationInFrameTree(
  ...
  // Document picture-in-picture activation propagation removed in this cut
  // along with PictureInPictureController: it exists to hand user
  // activation between an opener frame and a real, separate
  // picture-in-picture browser window, and a screenshot engine never opens
  // one.
  ...
  if (local_frame) {
    ...
    // Document picture-in-picture activation propagation removed in this
    // cut along with PictureInPictureController -- see the comment above,
...
bool Frame::ConsumeTransientUserActivationInFrameTree() {
  ...
  // Document picture-in-picture activation consumption removed in this cut
  // along with PictureInPictureController -- see the comment in
  // NotifyUserActivationInFrameTree() above.
...
void Frame::SetEmbeddingToken(const base::UnguessableToken& embedding_token) {
  ...
  // Used to notify the AXObjectCache when the embedding token (also used as
  // the AXTreeID referencing the child accessibility tree) changed; no
