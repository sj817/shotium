...
// Used to conditionally include "printing/metafile_skia.h" here for
// RemoteFrameView::Print(); see the comment on its removal in the header.
...
void RemoteFrameView::Paint(const PaintInfo& paint_info,
  ...
  if (owner_layout_object.GetDocument().IsPrintingOrPaintingPreview()) {
    ...
    // This used to compute `rect` (this view's size, placed at
    // DeprecatedLocation() unless AvoidEmbeddedContentViewLocation is on)
    // and branch on owner_layout_object.GetDocument().Printing():
    // when printing, RemoteFrameView::Print() (removed -- see
    // remote_frame_view.h) asked the browser process, over IPC, to print
    // this cross-process subframe and returned a placeholder content ID for
    // it; otherwise CapturePaintPreview() (removed along with
    // components/paint_preview) made the same kind of IPC ask for a paint
    // preview capture. Both branches needed a browser process to ask over
    // RemoteFrameHostRemote; a static screenshot engine is single-process
    // and has none, so there is no cross-process subframe capture or print
    // to perform either way. content_id stays the placeholder 0.
...
// RemoteFrameView::Print() removed in this cut -- see the comment at its
// former declaration in remote_frame_view.h.

// RemoteFrameView::CapturePaintPreview() removed in this cut -- see the
// comment at its former call site in Paint() above.
