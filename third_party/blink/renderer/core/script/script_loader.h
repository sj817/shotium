...
class CORE_EXPORT ScriptLoader final : public GarbageCollected<ScriptLoader>,
 ...
 public:
  ...
  // Not `override`: upstream's ScriptLoader derived from ResourceFinishObserver
  // (which declares a virtual Trace) so it could be notified when an external
  // script finished fetching. External script fetching is gone, and with it
  // NotifyFinished()/DebugName(), so the base is a plain GarbageCollected and
  // nothing above declares Trace.
 ...
 private:
  ...
  // AddSpeculationRuleSet()/RemoveSpeculationRuleSet() were here, for
  // <script type="speculationrules">. Speculation rules are prefetch and
  // prerender hints for a *next* navigation; core/speculation_rules is cut and
  // this engine performs exactly one navigation, so there is no next one to
  // speculate about.
