...
void ScriptLoader::Removed() {
  // This released the webbundle this loader had registered, ahead of GC. Web
  // bundles are gone; nothing else a ScriptLoader owns needs releasing early.
}

void ScriptLoader::DocumentBaseURLChanged() {
  // The only script type that cared about the base URL changing was
  // "speculationrules": its rule set holds resolved URLs, so it was reparsed
  // against the new base. Speculation rules are cut, and no other script type
  // keeps state that the base URL invalidates, so there is nothing to do.
}
...
// IsSameSite() was here. Its only caller decided whether a script was
// eligible for one of the parser-blocking interventions, and those went with
// script execution itself.

// IsDocumentReloadedOrFormSubmitted() and IsEligibleCommon() were here. They
// answered "is this document a candidate for the async-script delay
// intervention?", and the comment on DetermineDelayReasonsToWait() below
// records why that intervention can never fire in this engine. With the only
// caller gone the pair had nothing left to decide.

// The set of ScriptRunner::DelayReasons a newly-queued script should
// initially wait on. Upstream this can also include DelayReason::kMilestone
// (when `is_eligible_for_delay` holds -- the DelayAsyncScriptExecution
// intervention, crbug.com/1340837) or DelayReason::kPausedForPrerender (when
// a ScriptRunnerDelayer for it is active). Nothing in this engine ever
// constructs a ScriptRunnerDelayer -- there is no prerendering, and the
// milestone intervention was never wired past IsEligibleCommon() above,
// which nothing calls -- so those reasons would never be cleared, and
// including them here would permanently stall the script's execution
// (ScriptRunner::RemoveDelayReasonFromScript() only clears the bits it is
// told to clear; a bit nobody ever clears keeps the script queued forever).
// The only reason every script must wait for is that it has finished
// loading.
ScriptRunner::DelayReasons DetermineDelayReasonsToWait(
    ScriptRunner* script_runner,
    bool is_eligible_for_delay) {
  return static_cast<ScriptRunner::DelayReasons>(
  ...
  if (element_->HasSourceAttribute()) {
    ...
    switch (GetScriptType()) {
      ...
      case ScriptTypeAtPrepare::kClassic: {
        ...
        prepared_pending_script_ = ClassicPendingScript::Fetch(
      ...
      case ScriptTypeAtPrepare::kModule: {
        // - "module":
        //
        // Fetching an external module script graph meant parsing the module,
        // resolving its specifiers and fetching its dependency graph, all of
        // which existed only to hand the graph to V8 to instantiate and
        // evaluate. None of that survives, so an external module script is
        // handled exactly like an external classic script: no request, no
        // evaluation, and the same (always deferred or async, never
        // parser-blocking) scheduling that `GetScriptSchedulingTypePerSpec()`
        // derives from the element.
        prepared_pending_script_ = ClassicPendingScript::Fetch(
            url, element_document, options, cross_origin,
            element_document.Encoding(), element_, FetchParameters::kLazyLoad,
  ...
  if (!element_->HasSourceAttribute()) {
    ...
    switch (GetScriptType()) {
      ...
      case ScriptTypeAtPrepare::kImportMap: {
        ...
        // An import map only ever affects how a module specifier is resolved
        // while instantiating a module graph. There are no module graphs, so
        // parsing and registering the map would have no observable effect;
        // the element is recognised and otherwise ignored.
        return nullptr;
      }
      case ScriptTypeAtPrepare::kSpeculationRules: {
        // The rule set used to be parsed here and handed to
        // DocumentSpeculationRules, which turned it into prefetch and prerender
        // candidates for a *next* navigation. There is no next navigation in a
        // screenshot, so core/speculation_rules is cut and the element's
        // contents are simply not consumed. Returning null is what every
        // non-executing script type does here.
        return nullptr;
      }
      ...
      case ScriptTypeAtPrepare::kModule: {
        // Creating a JavaScript module script and fetching its descendant
        // graph existed only to hand the graph to V8. An inline module is
        // therefore kept only as source text, exactly like an inline classic
        // script, and is scheduled as a deferred script by
        // `GetScriptSchedulingTypePerSpec()` -- so it never blocks the parser
        // and its `load` event still fires.
        ...
        prepared_pending_script_ = ClassicPendingScript::CreateInline(
            element_, position, source_url, base_url, source_text,
            ScriptSourceLocationType::kInline, options,
            CaptureCurrentTaskState(context_window),
            CacheHintAttributeValue::kDefault,
            mojom::blink::ScriptType::kModule);
        break;
      }
    }
...
// AddSpeculationRuleSet()/RemoveSpeculationRuleSet() were here. They parsed
// <script type="speculationrules"> into a SpeculationRuleSet and registered it
// with DocumentSpeculationRules, which is cut.
