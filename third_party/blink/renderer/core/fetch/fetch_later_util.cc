...
uint64_t FetchLaterUtil::GetAvailableDeferredFetchQuota(Frame* frame,
  ...
  for (Frame* navigable = control_frame; navigable;
    ...
    // fetch group's deferred fetch records: there are none. Deferred fetch
    // records were only ever created by fetchLater(), which was reachable from
    // script alone, so nothing has consumed any of this quota.
  }
