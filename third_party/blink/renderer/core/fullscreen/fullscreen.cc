...
void FullscreenElementChanged(Document& document,
  ...
  // change the inertness of most elements. Used to reserialize the
  // accessibility tree for that; no accessibility tree exists to
  // reserialize anymore.
...
// GetErrorString() removed in this cut: it turned a RequestFullscreenError
// into a human-readable string to reject the ScriptPromise returned by
// requestFullscreen() with. There is no script engine and therefore no
// promise to reject, so its only caller is already gone and it was flagged
// as dead code (-Wunused-function).
...
void Fullscreen::RequestFullscreen(Element& pending,
  ...
  //
  // Element.requestFullscreen() returned a promise that resolved when the
  // browser had entered fullscreen and rejected with the reason otherwise.
  // There is no script to hand a promise to; the request itself, and every
  // fullscreenchange/fullscreenerror event it produces, is unchanged.
  ...
  if (for_cross_process_descendant) {
    ContinueRequestFullscreenAfterConditionsEnforcement(
        &pending, request_type, options, error);
  } else {
...
void Fullscreen::ContinueRequestFullscreenAfterConditionsEnforcement(
  ...
  if (error != RequestFullscreenError::kNone) {
    ...
    ContinueRequestFullscreen(document, *pending, request_type, options, error);
    return;
  }
...
void Fullscreen::ContinueRequestFullscreen(
  ...
  if (error != RequestFullscreenError::kNone) {
    ...
    // steps. There is no promise; the fullscreenerror event enqueued above is
  ...
  // 14. Resolve |promise| with undefined. There is no promise; the
  // fullscreenchange events enqueued above are the whole observable outcome.
}
...
void Fullscreen::FullyExitFullscreen(Document& document, bool ua_originated) {
  ...
  // ExitFullscreen() used to take one more argument between the
  // ExceptionState* and the ua_originated bool than it does now (see its
  // 3-arg declaration in fullscreen.h); drop the extra nullptr to match.
...
void Fullscreen::ExitFullscreen(Document& doc,
  ...
  //
  // document.exitFullscreen() returned a promise; there is no script left to
  // hand one to. The fullscreenchange events below are unchanged.
  ...
  // 7. Run the remaining steps in parallel.
  ...
  if (resize) {
    ...
    // will change observable state (document.fullscreenElement)
    // synchronously, so we have to continue asynchronously.
    doc.GetAgent().event_loop()->EnqueueMicrotask(BindOnce(
        ContinueExitFullscreen, WrapPersistent(&doc), false /* resize */));
  }
}
...
void Fullscreen::DidExitFullscreen(Document& document) {
  ...
  wtf_size_t exits = std::exchange(fullscreen.pending_exits_, 0u);
  if (!exits) {
    FullyExitFullscreen(document, true /* ua_originated */);
  } else {
    for (wtf_size_t i = 0; i < exits; ++i) {
...
Fullscreen::PendingRequest::PendingRequest(Element* element,
                                           ...
                                           const FullscreenOptions* options)
    : element_(element), type_(type), options_(options) {}
