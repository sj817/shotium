...
class CONTROLLER_EXPORT BlinkLeakDetector : public mojom::blink::LeakDetector {
 ...
 private:
  ...
  void ReportResult();
  // ReportInvalidResult() was here, called only from a worker-thread-count
  // bail-out in PerformLeakDetection(). There are no workers any more, so
  // that bail-out (and this, its only caller) is gone; see the comment in
