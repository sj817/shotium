...
class WebAudioSinkDescriptor {
 public:
  ...
  bool IsDefaultSinkId() const {
    // media::AudioDeviceDescription::kDefaultDeviceId, spelled out because
    // //media/audio is cut. It is the string "default" -- a wire value fixed
    // by the Web Audio spec's sinkId, not an internal choice that could drift.
    return SinkId().IsEmpty() || (SinkId() == "default");
  }
