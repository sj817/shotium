...
class WebWorkerFetchContext : public base::RefCounted<WebWorkerFetchContext> {
 public:
  ...
  // SetSubresourceFilterBuilder()/TakeSubresourceFilter() were here: the
  // embedder handed over a builder on the main thread and the worker thread
  // built the filter from it. Nothing implements WebDocumentSubresourceFilter
  // in this build and nothing ever set a builder.
