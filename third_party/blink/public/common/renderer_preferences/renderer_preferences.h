...
struct BLINK_COMMON_EXPORT RendererPreferences {
  ...
  // webrtc_ip_handling_urls was here: per-origin overrides of the policy
  // above, each a ContentSettingsPattern plus a policy. The entry struct
  // lived in public/common/peerconnection, which is cut, and its mojom
  // typemap went with it. The global policy is the only policy now.
