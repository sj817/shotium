...
class BLINK_EXPORT WebLocalFrame : public WebFrame {
 public:
  ...
  // SetContentCaptureClient()/ContentCaptureClient() were here; content
  // capture is cut.
  ...
  // GetAXTreeID() used to return the ui::AXTreeID tied to this frame's
  // accessibility tree (via its EmbeddingToken); the accessibility tree
  // (AXObjectCache, and the ui::AXTreeID/AXTreeUpdate/... node/tree model)
  // was removed, so there is no tree to identify anymore.
  ...
  // AddMessageToConsole() was here: the embedder's way of writing into the
  // frame's console. It took a WebConsoleMessage, and ConsoleMessage's
  // constructor for that type went with the embedder-facing API. Blink's own
  // console messages (FrameConsole, Document::AddConsoleMessage) are
  // untouched; what is gone is the door from outside.

  // AddInspectorIssue()/AddUserReidentificationIssue()/AddGenericIssue() were
  // here. They filed a DevTools "Issues" panel entry from the embedder; the
  // only caller was the web-test runner. core/inspector's audits reporter and
  // the DevTools protocol are cut.

  // Alert()/Confirm()/Prompt() exposed the modal dialog methods to avoid
  // having to go through JavaScript; no embedder ever called them (there is
  // no content-layer WebLocalFrameClient consumer in this build) and no
  // implementation exists, so the pure-virtual declarations were removed
  ...
  // Ad Tagging is gone. IsAdFrame(), SetAdEvidence(), AdEvidence(),
  // IsAdScriptInStack() and IsFrameCreatedByAdScript() were here. SetAdEvidence()
  // was the embedder's only way in, and its only caller was SubresourceFilterAgent
  // in content/renderer, deleted with //components/subresource_filter; the other
  // four answered from AdTracker, which walked the V8 stack.
