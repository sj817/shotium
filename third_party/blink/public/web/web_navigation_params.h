...
struct BLINK_EXPORT WebNavigationInfo {
  ...
  // initiator_frame_is_ad and is_ad_script_in_stack were here. Both came from
  // the initiator LocalFrame's ad status, which no longer exists: ad tagging
  // needed either AdTracker's V8 stack walk or the subresource filter.
...
struct BLINK_EXPORT WebNavigationParams {
  ...
  // service_worker_network_provider was here; service workers are cut.
