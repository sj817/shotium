...
// See the comment on the declaration in include/cppgc/heap-statistics.h: this
// is the forwarder v8::CppHeap::CollectStatistics() used to be.
HeapStatistics CollectStatistics(HeapHandle& heap_handle,
                                 HeapStatistics::DetailLevel detail_level) {
  return internal::HeapBase::From(heap_handle).CollectStatistics(detail_level);
}
