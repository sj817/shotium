...
/**
 * Collects statistics for the heap `heap_handle` refers to.
 *
 * Added for the standalone build. Upstream this lived on the internal
 * `cppgc::internal::HeapBase`, and embedders reached it through
 * `v8::CppHeap::CollectStatistics()` -- which did nothing but forward. With V8
 * gone there is no forwarder, and an embedder calling `HeapBase::From()`
 * directly has to include `src/heap/cppgc-internal/heap-base.h`, which drags
 * cppgc's own `src/base/logging.h` into the caller and redefines CHECK,
 * CHECK_OP, PRINTF_FORMAT and friends out from under it.
 *
 * So the forwarder is here instead, taking the `HeapHandle` an embedder
 * already has.
 */
V8_EXPORT HeapStatistics CollectStatistics(HeapHandle& heap_handle,
