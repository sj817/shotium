...
std::unique_ptr<HWNDMessageHandler> HWNDMessageHandler::Create(
  ...
  // HWNDMessageHandlerHeadless overrode this for //headless's virtual displays,
  // where a window has no real HWND geometry to report. That component is gone
  // and IsHeadless() is now always false, so there is one handler.
  return base::WrapUnique(new HWNDMessageHandler(delegate, debugging_id));
}
