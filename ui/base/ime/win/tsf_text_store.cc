...
HRESULT TSFTextStore::GetACPFromPoint(TsViewCookie view_cookie,
  ...
  // When StylusHandwritingWin was enabled and the `view_cookie` was unknown,
  // this queried the "proximate" bounds cache as a fallback and yielded a
  // character index. components/stylus_handwriting is deleted, so the feature
  // cannot be enabled and the fallback is unreachable; an unknown view cookie
  // is now always an invalid argument.
  return E_INVALIDARG;
}
...
HRESULT TSFTextStore::GetTextExt(TsViewCookie view_cookie,
  ...
  if (!text_input_client_)
    return E_UNEXPECTED;
  // StylusHandwritingWin used to make an unknown `view_cookie` acceptable here,
  // answering from the "proximate" bounds cache further down.
  // components/stylus_handwriting is deleted, so the only view is ours.
  if (view_cookie != kViewCookie) {
    return E_INVALIDARG;
  }
  if (!HasReadLock())
  ...
  // The StylusHandwritingWin branch that answered an unknown `view_cookie` from
  // GetProximateCharacterBounds() is gone with components/stylus_handwriting.
  // The early return above rejects that case before reaching here.
