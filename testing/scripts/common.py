...
def set_lpac_acls(acl_dir, is_test_script=False):
  ...
  if sys.getwindowsversion().build < 19041:
    return
  try:
    # icacls writes in the console's ANSI code page, not UTF-8. On a localized
    # Windows (for example zh-CN, where it emits GBK) universal_newlines=True
    # decodes as UTF-8 and raises UnicodeDecodeError before the build can even
    # start. The only thing read out of this output is the ASCII SID strings in
    # CORRECT_ACL_VARIANTS, so replacing undecodable bytes is lossless here.
