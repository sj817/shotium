...
URLRequestContextOwner NetworkContext::MakeURLRequestContext(
  ...
  // A prepare_upload_request_callback was installed here to stamp variations
  // headers onto Reporting API uploads. //components/variations is deleted, so
  // there are no field trial ids to announce and the uploads go out unstamped.
...
// SetVariationsHeaders() was here; the mojo method went with
