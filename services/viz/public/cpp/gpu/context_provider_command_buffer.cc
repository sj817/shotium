...
gpu::ContextResult ContextProviderCommandBuffer::BindToCurrentSequence() {
  ...
  switch (attributes_->which()) {
    case gpu::mojom::ContextCreationAttribs::Tag::kWebgpu: {
      // WebGPUCmdHelper/WebGPUImplementation are gone with Dawn/WebGPU;
      // there is no implementation left to bind here.
      DLOG(ERROR) << "WebGPU is not supported.";
      bind_result_ = gpu::ContextResult::kFatalFailure;
      return bind_result_;
    }
...
gpu::webgpu::WebGPUInterface* ContextProviderCommandBuffer::WebGPUInterface() {
  // WebGPUInterface is gone with Dawn/WebGPU; BindToCurrentSequence() always
  // fails for kWebgpu contexts, so this is never actually reachable with
  // bind_result_ == kSuccess.
  return nullptr;
}
