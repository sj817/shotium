...
Node.js 服务端集成示例：演示 Express 通过 HTTP 托管[角色资料卡片](../demo-genshin-card/README.zh.md)页面，使用 `@shotkit/shotium` 截取目标 DOM 元素，并完成进程优雅退出。
...
本示例直接引用本地构建产物 `../typescript/dist/index.js` 以测试当前仓库代码。在实际应用项目中，直接从 npm 安装 `@shotkit/shotium` 依赖即可。
