...
| `build/config/compiler/BUILD.gn` ThinLTO 段 | `is_shot_build` 不加 `-fsplit-lto-unit` 和 `-fwhole-program-vtables`，Rust 只保留 `-Clinker-plugin-lto=yes`、不加 `-Zsplit-lto-unit` | 这对旗标是给整程序去虚化和 CFI 用的。Windows 从来不传对应的链接旗标，去虚化没跑过，但类型测试降级仍把同一继承树的全部 vtable 拼成一个全局；/OPT:REF 因此删不掉任何有兄弟在用的 vtable 及其虚函数，2026-09-17 的 shotium.exe 里 90% 的 vtable 字节在这种分组里，拖着约 2.2 MB 死代码（FreeType、ICU 格式化/排序、re2、perfetto 服务）。六平台同一设置；Linux/mac 少的是去虚化，Chrome 量过约 1% Speedometer，不是行为 |
| `skia/features.gni` / `build/args/shot.gn` / Blink 与 cc 的 gainmap 路径 | `skia_support_xmp` 改成 declare_args 并在 shot 关掉；`DeferredImageDecoder`、`JPEGImageDecoder`、`BitmapImage`、`ImageDecoder` 删除 gainmap 提取，`DrawImageOp`/`DrawImageRectOp` 不再走 `skia::DrawGainmapImage`，`skia/ext/draw_gainmap_image.*` 删除 | 合成到 SDR 时 gainmap 权重为 0，只有底图可见，像素不变；去掉的是 SkGainmapShader→SkSL 运行时效果、XMP/expat、SkDOM 和 SVG canvas device。全局 tone mapping（PQ/HLG、PNG cICP）保留，那条路径影响像素 |
| `Node::DefaultEventHandler` / `EventHandler` / `Frame` / `Editor` / `FrameSelection` | 删除 keydown/keypress/keyup 和 textInput 的默认处理及 `DefaultKeyboardEventHandler`、`DefaultTextInputEventHandler`；删除 `Frame::SetTextDirection`、`Editor::SetBaseWritingDirection`；`FrameSelection` 不再调 `TypingCommand::CloseTyping*`（两个静态一并删） | 没有任何键盘、文本输入、拖放或 execCommand 入口，`Editor::LastEditCommand()` 恒为空。这五处是编辑命令栈（TypingCommand、SpellChecker、IME、ApplyStyle、ReplaceSelection……约 0.45 MB）仅剩的活引用；选区、可见单位、标记等布局/绘制要用的编辑基础设施保留 |
| `core/html/forms/type_ahead.{h,cc}` 及 `HTMLSelectElement` / `HTMLMenuOwnerElement` / `DateTimeSymbolicFieldElement` | TypeAhead 整个删除，三个宿主去掉 `TypeAheadDataSource` 基类、成员和键盘分支；`String::StartsWithIgnoringCaseAndAccents` 删除 | 键盘联想选项只有键盘事件能触发；它是 ICU 排序（ucol/usearch，0.16 MB 代码 + coll/ 数据）在这个二进制里唯一的可达用户 |
| `platform/wtf/date_math.cc` `ConvertToLocalTime` | 用 `base::Time::LocalExplode`+`FromUTCExploded` 求本地偏移，不再 `icu::TimeZone::createDefault` | 只有 date/time 表单控件 stepUp 的默认值调用它，毫秒精度足够；它是 icu::TimeZone 的最后调用者，去掉后 zone/ 数据和时区代码都能删 |
| ICU `shot` 数据集（`scripts/build/icu-repack.ts`） | `cast` 去掉 zone/、lang/、region/、curr/、coll/ 五棵资源树（1.95 MB）；全部转换表保留 | 每棵树对应一个 ICU 服务（TimeZoneNames、uldn、ucurr、ucol），最终链接的段级引用图里都没有活调用方；Linux/mac 的 LocaleICU 只读保留的 `<locale>.res`、zoneinfo64 和 brkitr。六张 CJK 转换表（853 KB）看起来没人开，其实是 `wtf/text/encoding_tables.cc` 运行时 `ucnv_open` 建 TextCodecCjk 解码表的数据源，release 版缺表不是 U+FFFD 而是未初始化值写越界；legacy charset 现在真的会解码，所以必须留 |
| `net/cert/cert_verify_proc_builtin.cc`；`net/cert/ev_root_ca_metadata.{h,cc}` 删除；`net/data/ssl/chrome_root_store/BUILD.gn` 与 `root_store_tool` 不再生成 `chrome-ev-roots-inc.cc` | 内建校验器只做 DV：没有 `VerificationType`、EV 候选判断、EV 策略集和 `kEV` 路径构建尝试，`CERT_STATUS_IS_EV` 永远不置位 | EV 状态只有浏览器地址栏读，shot 没有消费方。上游 Linux 总带 NSS，Linux 关 NSS 后 `EVRootCAMetadata` 落到桩实现，每个策略 OID 查一次就打一行「Not implemented」，每条 HTTPS 连接刷屏；Windows/mac 则为 EV 叶子多跑一整轮路径构建，结果没人看。校验结果、吊销策略和 CT 检查不变 |
| `core/frame/local_frame.{h,cc}` `ForceSynchronousDocumentInstall` / `shot/shot_renderer.cc` | 多一个带 `encoding` 形参的重载，shot 把 Content-Type 的 charset 作为 header 编码传进去；上游三个重载不变，仍写死 UTF-8 | 上游这条路只给 SVG 图片和内部页用，写死 UTF-8 没问题；shot 用它装顶层文档，写死意味着 BOM、header charset、`<meta charset>` 全部被忽略（`pnpm verify:charset` 之前 11 个用例全是 mojibake）。传 header 编码后优先级就是浏览器的：BOM > header > meta > CED 嗅探 > 顶级域默认 > Settings 默认 |
| `build/args/shot.gn` `include_transport_security_state_preload_list = false`；`net/tools/{transport_security_state_generator,huffman_trie}` 与 `net/http/transport_security_state_static.*` 只留 BUILD.gn；`net/http/BUILD.gn` 的三个 `compiled_action` 和 `net/BUILD.gn` 里测试目标对它们的 deps 都套在 `if (include_transport_security_state_preload_list)` 里 | 不带 HSTS 预加载表和公钥 pin；生成器的 action 只在要表时定义 | Cronet 的既有配置，`transport_security_state.cc` 里 `DecodeHSTSPreload` 直接返回 false。action 必须一起套条件：上游只是不 dep 它，但一条没人引用的 action 只要输入文件不在、而旧 `.ninja_deps` 里还记着它的输出，ninja 就报 missing and no known rule。0.77 MB 纯数据，外加每次冷构建都要编一遍的宿主工具（含宿主工具链的 BoringSSL）和 10.5 MB 的 JSON。差别只在页面写 `http://` 引用预加载域名时多一次 301 往返；同进程内见过的 Strict-Transport-Security 头仍生效 |
| `platform/fonts/skia/font_cache_skia.cc` `FontCache::CreateTypeface`；`platform/fonts/linux/font_unique_name_lookup_linux.cc` `MatchUniqueName` | 按 fontconfig id 创建字体的分支改成「有文件路径就按路径打开，没有才按 id」；上游是「有 WebSandboxSupport 就按 id」 | Chrome 的渲染进程开不了字体文件，id 由浏览器进程的字体服务分配；shot 的 `ShotSandboxSupport`（v0.10.0 为了 `local()` 加的）在进程内查 fontconfig，填的是真实路径，id 永远是 0。按上游的分支走，`SkFontConfigInterfaceDirect::openStream("")` 返回空，系统回退字体全部创建失败：Linux 上主字体没有的 CJK 全是方框、emoji 全是空白，v0.10.0 到 v0.11.1 都如此。`verify:charset`、`verify:demos` 这类比较式检查看不出来，方框等于方框；`pnpm verify:fonts` 专门守这条 |
