...
// Wraps a persistent_cache::PersistentCache to be used as a Skia or ANGLE
// cache. (It used to also back a Dawn cache via
// dawn::platform::CachingInterface; that's gone with Dawn.) Entries are
// always stored in a MemoryCache and PersistentCache as well once it is
// initialized. Entries loaded before the PersistentCache is initialized are
// copied into it on initialization.
class GPU_GLES2_EXPORT GpuPersistentCache
 ...
 public:
  ...
  // General string-keyed load/store API, used directly by tests and by
  // callers that don't go through the Skia/ANGLE-specific helpers below.
