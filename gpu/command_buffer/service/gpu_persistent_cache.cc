...
bool IsVkPipelineCache(std::string_view key_str) {
  // Vulkan appends a suffix to the cache key intentionally.
...
// FindKey/LoadData(std::span<const std::byte>, ...) overloads used to adapt
// this string_view-keyed API to dawn::platform::CachingInterface; gone with
// Dawn.
...
// StoreData(std::span<const std::byte>, ...) used to adapt this
// string_view-keyed API to dawn::platform::CachingInterface; gone with Dawn.
