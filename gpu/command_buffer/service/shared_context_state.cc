...
// X11 Xlib.h defines Status as int and X.h defines Success as 0, both of
// which conflict with enum values used elsewhere in this file.
#undef Status
#undef Success
...
gpu::GraphiteSharedContext* SharedContextState::graphite_shared_context()
    const {
  // GraphiteDawn is gone with Dawn; there is no other GraphiteSharedContext
  // source yet.
  return nullptr;
}
...
// IsGraphiteDawnMetal/D3D/D3D11/Vulkan/VulkanSwiftShader used to query
// dawn_context_provider()->backend_type(); Dawn is gone, and
// IsGraphiteDawn() above is now always false, so these can never be true.
...
bool SharedContextState::InitializeGraphite(
  ...
  // This path used to initialize a Dawn-backed GraphiteSharedContext via
  // dawn_context_provider_. Dawn is gone, so there is no way to satisfy a
  // kGraphiteDawn request; fall through to the failure handling below.
...
std::optional<error::ContextLostReason> SharedContextState::GetResetStatus(
  ...
  // This used to query dawn_context_provider_->GetResetStatus() for
  // GraphiteDawn; Dawn is gone, so that path is no longer reachable.
...
int32_t SharedContextState::GetMaxTextureSize() {
  ...
  if (IsUsingGL()) {
    ...
    // This used to query dawn_context_provider()->GetDevice().GetLimits()
    // for GraphiteDawn. Dawn is gone, and GraphiteDawn contexts never
    // initialize successfully now, so this branch is unreachable in
    // practice.
  }
...
Microsoft::WRL::ComPtr<ID3D11Device> SharedContextState::GetD3D11Device()
  ...
  switch (gr_context_type_) {
    ...
    // GraphiteDawn used to return dawn_context_provider_->GetD3D11Device()
    // here; Dawn is gone, and GraphiteDawn contexts never initialize
    // successfully now, so this falls through to NOTREACHED() below.
    default:
      NOTREACHED();
  }
}
