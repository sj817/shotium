...
class GPU_GLES2_EXPORT D3DImageBacking final
 ...
 private:
  ...
  // Common state tracking for D3D11/D3D12 access.
  ...
  // Returns the fences `d3d11_device` must wait on before it can access the
  // backing: the previous write's fence(s) always, the previous read
  // fence(s) as well if `write_access`, and (when cross-device fence
  // synchronization is in use) any fence a *different* device signaled on
  // its own EndAccess that `d3d11_device` hasn't necessarily observed yet.
  std::optional<D3DSharedFenceSet> GetPendingWaitFences(
  ...
  // underlying d3d texture is gone with Dawn.
  ...
  // The persistent Graphite Dawn SharedTextureMemory access that used to be
  // kept open here is gone with Dawn.

  class GraphiteTextureHolder;

  // Used to cache the last signaled fence from Dawn buffer access, keeping
  // the underlying NT handle open to avoid OpenSharedHandle calls. No longer
  // set now that Dawn is gone.
