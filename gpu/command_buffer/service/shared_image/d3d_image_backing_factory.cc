...
bool D3DImageBackingFactory::IsSwapChainSupported(
  ...
  if (gpu_preferences.gr_context_type == GrContextType::kGraphiteDawn) {
    // This used to check that graphite and ANGLE share the same D3D11
    // device via dawn_context_provider->GetD3D11Device(). Dawn is gone, so
    // `dawn_context_provider` is always null now; there's no way to satisfy
    // this request.
    return false;
  }
