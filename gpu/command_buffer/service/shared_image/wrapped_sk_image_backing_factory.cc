...
SharedImageUsageSet GetSupportedUsage(const SharedContextState* context_state) {
  // This used to add a WebGL/WebGPU fallback usage set when using Graphite
  // Dawn Vulkan or D3D12. Dawn is gone, and IsGraphiteDawn() is now always
  // false, so kSupportedUsage is all that's left.
  return kSupportedUsage;
}

bool GraphiteSupportsCompressedTextures(
    const SharedContextState* context_state) {
  // This used to query Dawn for compressed-texture support when
  // IsGraphiteDawn(). Dawn is gone, and IsGraphiteDawn() is now always
  // false.
  return false;
}
