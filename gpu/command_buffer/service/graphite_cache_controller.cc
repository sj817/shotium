...
GraphiteCacheController::GraphiteCacheController(
    ...
    : recorder_(recorder),
      ...
      // Dawn is gone, so there is no DawnContextProvider to hand context
      // resource cleanup off to.
...
void GraphiteCacheController::CleanUpAllResourcesImpl() {
  ...
  // This used to report progress to the GPU watchdog thread via
  // dawn_context_provider_->GetDawnPlatform() during long memory-reduction
  // operations. Dawn is gone, and can_handle_context_resources_ is always
  // false now, so this is unreachable anyway.
}

GraphiteSharedContext* GraphiteCacheController::GetGraphiteSharedContext() {
  // GraphiteDawn is gone; there is no other GraphiteSharedContext source yet.
  return nullptr;
}
