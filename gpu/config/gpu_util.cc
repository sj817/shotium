...
GpuFeatureStatus GetVulkanFeatureStatus(
  ...
  // TODO(crbug.com/522211393): SKIA_USE_DAWN is never defined now that Dawn
  // is gone, so if Graphite is enabled we always fall back to GL instead of
  // Vulkan. This was an old behavior, failing to keep it would lead to test
  // failures.
...
GpuFeatureStatus GetWebNNFeatureStatus(
    const std::set<int>& blocklisted_features) {
  // //services/webnn is gone, so WebNN can never be enabled here.
  return kGpuFeatureStatusDisabled;
}
