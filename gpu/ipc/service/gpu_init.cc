...
// No-op now that Dawn is gone; kept so call sites don't need to change.
...
bool GpuInit::InitializeAndStartSandbox(base::CommandLine* command_line,
...
#if BUILDFLAG(IS_WIN)
  {
    ...
    if (base::PathService::Get(base::DIR_MODULE, &module_path)) {
      ...
      // (DefaultForceFallbackAdapter() used to also cover Graphite/Dawn using
      // SwiftShader via --skia-graphite-dawn-backend=swiftshader; gone with
      // Dawn.)
...
#if BUILDFLAG(IS_WIN)
  {
    ...
    // This used to prefer dawn_context_provider_'s D3D11/D3D12 devices when
    // Graphite-Dawn was active. Dawn is gone, so always query ANGLE.
...
#endif  // BUILDFLAG(ENABLE_VULKAN)
  // This used to also set a DrmModifiersFilterDawn on ChromeOS when
  // dawn_context_provider_ was present; gone with Dawn.
#endif  // BUILDFLAG(IS_OZONE)
...
#if !BUILDFLAG(IS_ANDROID)
  if (gpu_preferences_.gr_context_type == GrContextType::kGraphiteDawn) {
    // InitializeDawn() always fails now that Dawn is gone. This used to be
    // LOG(FATAL) when SkiaGraphite was enabled (no fallback was expected to
    // be needed), but a request for GraphiteDawn can no longer ever be
    // satisfied, so fall back to GL instead of crashing the GPU process.
    if (!InitializeDawn()) {
      gpu_preferences_.gr_context_type = GrContextType::kGL;
    }
  }
...
void GpuInit::SetSkiaBackendType() {
  ...
  switch (gpu_preferences_.gr_context_type) {
    ...
    case gpu::GrContextType::kGraphiteDawn:
      // Dawn is gone, and both InitializeAndStartSandbox() and
      // InitializeInProcess() fall back to kGL before reaching here, so this
      // should be unreachable. Fail gracefully rather than crash if it's
      // somehow reached anyway.
      LOG(ERROR) << "GraphiteDawn was requested but Dawn is gone.";
      break;
  }
...
bool GpuInit::InitializeDawn() {
  // This used to create a DawnContextProvider (gpu_preferences_,
  // gpu_feature_info_, a progress reporter for the watchdog, and an adapter
  // validation callback all fed into it). Dawn is gone, so this can never
  // succeed; callers already treat a false return as "no GraphiteDawn" and
