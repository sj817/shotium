...
scoped_refptr<gl::Presenter> ImageTransportSurface::CreatePresenter(
  ...
  if (gl::DirectCompositionSupported()) {
    // This used to DCHECK that DirectComposition was only reached on
    // Graphite's Dawn D3D11 backend. Dawn is gone, and IsGraphiteDawn() is
    // now always false, so that invariant no longer applies.
