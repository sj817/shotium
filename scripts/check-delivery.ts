...
async function main(platformDirectory: string): Promise<void> {
  ...
  await execa(npm, ['pack', '--pack-destination', temporary], {cwd: resolve('apps/demo/shotium')});
  ...
  const source = `
    ...
    (async () => {
      ...
      const result = await shot.screenshot({file: process.env.SHOT_FIXTURE, allowFileAccess: true, viewport: {width: 800, height: 600}});
      assert.equal(hash(result.image), process.env.SHOT_EXPECTED);
      assert.equal(result.stats.failed, 0);
      const tiles = await shot.screenshotTiles({file: process.env.SHOT_FIXTURE, allowFileAccess: true, viewport: {width:800,height:600}, tile: {height:300}});
  ...
  const fixture = resolve('apps/fixtures/hello.html');
  await execa(path.join(platformPath, process.platform === 'win32' ? 'shotium.exe' : 'shotium'), ['--file', fixture, '--width', '800', '--height', '600', '-o', cliPng]);
