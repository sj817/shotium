...
const cli = cac('pnpm package:c-abi');
cli.command('', 'stage a C ABI Release directory; paths are relative to the repository root')
    ...
    .action((options: {build: string; dest?: string; os?: string}) => {
      try {
        ...
        files.push([resolve('shot/shot_api.h'), 'shot_api.h'], [resolve('apps/c-abi/README.md'), 'C_ABI.md'], [resolve('apps/c-abi/README.zh.md'), 'C_ABI.zh.md'], [resolve('LICENSE'), 'LICENSE']);
