...
// script is `pnpm -C scripts exec tsx build/engine.ts`, so a second pnpm runs and
...
const cli = cac('pnpm build:engine')
...
// one level down (`pnpm -C scripts exec tsx build/engine.ts`), so resolve against the
