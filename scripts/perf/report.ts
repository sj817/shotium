...
import {acceptanceMode, assessAcceptance, type AcceptanceMode, type AcceptanceResult, type LoadedRuntime} from './acceptance.ts';
...
interface Result extends AcceptanceResult {
  platform: string;
  arch: string;
  revision: string;
...
export function report(files: string[], output: string, platforms: Set<string> = PLATFORMS, mode: AcceptanceMode = 'improvement'): boolean {
  const identical = mode === 'identical-runtime';
  const lines: string[] = [
    '# 本次候选构建与 npm 的完整矩阵验收', '',
  ...
  for (const file of files) {
    ...
    const issues = assessAcceptance(data, mode);
    if (!/^[0-9a-f]{40}$/.test(data.revision) || [data.sourceDiffSha256, data.harnessSha256, data.fixtureManifestSha256].some((s) => !/^[0-9a-f]{64}$/.test(s))) {
      issues.push('Missing or invalid source, harness or fixture provenance');
    }
    ...
    if (complete && !issues.length && pixelOk) passing.add(platform);
    lines.push(
        `## ${platform}`, '',
        `覆盖：${observed.size}/${required.size}；性能提升通过：${accepted}/${required.size}；完整采样：${complete ? '是' : '否'}；` +
            `独立像素检查：${pixelOk ? '通过' : '未通过或未运行'}。`, '',
        `本平台 ${mode} 验收：${passing.has(platform) ? '通过' : '未通过'}。${issues.length ? `原因：${issues.join('；')}。` : ''}`, '',
        `噪声带：中位/均值 +${f2(bodyBand * 100)}%，尾部 +${f2(tailBand * 100)}%（${bandSource}）。` +
            ...
            `性能提升检查中，引擎场景须「更快」才算通过；${external} 项被外部等待占死的场景（服务端固定延迟、静默窗口）只要求不更慢。表中 ✓/✗ 始终表示性能提升检查；以下均为毫秒。`, '',
    ...
    for (const [label, meta] of Object.entries(data.metadata)) {
      ...
      if (identical) {
        const before = data.runtimeIdentity?.[label]?.before, after = data.runtimeIdentity?.[label]?.after;
        lines.push(`  运行文件 ${before?.files.length ?? 0} 个；采样前 SHA256 \`${before?.sha256 ?? '未记录'}\`；采样后 \`${after?.sha256 ?? '未记录'}\`。`);
      }
    }
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  const cli = cac('pnpm perf:report');
  cli.command('<...files>', 'render the acceptance report from perf-compare results')
      ...
      .action((files: string[], options: {output?: string; platform?: string | string[]; acceptance?: string}) => {
        ...
        try {
          process.exitCode = report(files.map((f) => resolve(f)), resolve(options.output), chosen ?? PLATFORMS, acceptanceMode(options.acceptance)) ? 0 : 1;
        } catch (error) {
