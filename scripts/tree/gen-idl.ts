...
// and the `all` sweep recognise the banner by this prefix, and the
// subcommand after it is one token so the prune regexes can capture it. It
// names the command rather than the script's path: the command survives the
// script moving, and the 400 generated files would otherwise have to be
// rewritten every time it does.
const GENERATOR = {enums: 'pnpm gen:idl enums', unions: 'pnpm gen:idl unions', dictionaries: 'pnpm gen:idl dictionaries', buildGni: 'pnpm gen:idl build-gni'};
const BANNER_PREFIX = 'pnpm gen:idl ';
...
# scripts/tree/gen-idl.ts (pnpm gen:idl). Upstream produced these from the same .idl files
