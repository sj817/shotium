...
//
// The fixtures in apps/test/render/cases contain no JavaScript, animation, clocks,
...
cli.command('run', 'render every case with the engine and compare it with the baseline')
    ...
    .option('--cases <file>', 'the case manifest', {default: 'apps/test/render/cases.json'})
    .option('--baselines <dir>', 'where the baselines are', {default: 'apps/test/render/baselines'})
    .option('--artifacts <dir>', 'where renders, diffs and report.json go', {default: 'apps/test/render/out'})
...
cli.command('update-baselines', 'replace the baselines with renders from a pinned engine')
    ...
    .option('--cases <file>', 'the case manifest', {default: 'apps/test/render/cases.json'})
    .option('--baselines <dir>', 'where the baselines go', {default: 'apps/test/render/baselines'})
    .option('--timeout-ms <ms>', 'per-render timeout', {default: 30000})
    .option('--sampling-interval-ms <ms>', 'process sampling interval', {default: 10})
