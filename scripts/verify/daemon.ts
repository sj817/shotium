...
// started it.
//
// check-node covers the engine in this process, and the daemon is the same
// engine in one of its own -- so what is left to check here is the part that
// is genuinely new. That a client in another process finds the daemon instead
// of starting a second one; that one connection can carry several requests at
...
async function main(exeArg: string): Promise<number> {
  ...
  // The resource packs sit beside the executable in a checkout rather than
  // beside the addon. An install has them in one directory and needs none of
  // this.
  const buildDir = path.dirname(exe);
  const corpus = resolve('shot/testdata/render_corpus.html');
  const entry = pathToFileURL(resolve('apps/typescript/dist/index.js')).href;
  const shotium = createRequire(import.meta.url)(resolve('apps/typescript')) as typeof Shotium;
  const checks = new Checks();
  const check = checks.check.bind(checks);

  // A second node process, which is what this check is actually about: a
  // caller in a process that did not start the daemon has to find it rather
  // than start another one. It is spelled out here rather than run from a
  // script in the package because the package no longer ships one -- the
  // command line is a binary of its own now -- and this is what a caller
  // writes anyway.
  //
...
const {image, stats} = await shotium.daemon.screenshot({...request, daemon: config});
writeFileSync(output, image);
process.stdout.write(JSON.stringify({bytes: image.length, elapsedMs: Date.now() - started, requests: stats.requests}));
`;

  // A name of this run's own, so that a daemon left over from an earlier run
  // -- or a developer's own, started by hand -- can never be mistaken for the
  // one under test.
  const config = {
  ...
  check(before.running === false, 'status reports no daemon at the endpoint', before.endpoint);
  check(before.endpoint.includes(`-${config.name}-v${PROTOCOL_VERSION}`), 'a named endpoint includes the wire generation', before.endpoint);

  checks.section('the first request starts one');
  ...
  check(Buffer.isBuffer(first) && first.subarray(0, 8).equals(Buffer.from('89504e470d0a1a0a', 'hex')), 'a screenshot comes back as a PNG');
  ...
  check(stats !== undefined && stats.requests >= 1 && stats.timing.total > 0, 'with the statistics the in-process engine reports', `${stats.requests} request(s), ${stats.timing.total.toFixed(1)}ms`);
  const started = await client.status();
  check(started.ok === true && started.pid > 0, 'and it is a process of its own', `pid ${started.pid}, ${cold}ms to connect`);
  ...
  writeFileSync(tall, '<body style="margin:0"><div style="height:35990px;background:#fff"></div><div style="height:10px;background:#f00"></div></body>');
  const {tiles, stats: tileStats} = await client.screenshotTiles({file: tall, viewport: {width: 400, height: 300}, fullPage: true, tile: {height: 8000}});
  rmSync(tall, {force: true});
  check(tiles.length === 5 && tiles.every((t) => Buffer.isBuffer(t.image) && t.image.length > 0), 'five tiles come back, each with its image', `${tiles.length}`);
  check(tiles.map((t) => t.image!.readUInt32BE(20)).join() === '8000,8000,8000,8000,4000', 'each as tall as its tile', tiles.map((t) => t.image!.readUInt32BE(20)).join());
  check(tileStats !== undefined && tileStats.requests >= 1, 'with the statistics in the header');
  const {image: afterTiles} = await client.screenshot(request);
  check(Buffer.isBuffer(afterTiles) && afterTiles.equals(first!), 'and the next screenshot on the same socket is still in step');

  checks.section('it is the same renderer as an engine in this process');
  ...
  await runtime.stop();
  check(sha256(inProcess!) === sha256(first!), 'the daemon and an in-process engine produce the same bytes', sha256(first!).slice(0, 16));

  checks.section('a second process finds it instead of starting one');
  const output = path.join(os.tmpdir(), `shotium-daemon-check-${process.pid}.png`);
  const secondStarted = Date.now();
  const printed = execFileSync(process.execPath, ['--input-type=module', '-e', SECOND_PROCESS, JSON.stringify([config, request, output])], {encoding: 'utf8'});
  const secondWall = Date.now() - secondStarted;
  const reported = JSON.parse(printed) as {elapsedMs: number};
  const after = await client.status();
  check(after.pid === started.pid, 'the daemon that answered is the one that was already up', `pid ${after.pid}`);
  check(after.served > started.served, 'and it counted the request', `${started.served} -> ${after.served}`);
  const written = readFileSync(output);
  check(sha256(written) === sha256(first!), 'the image it wrote is the same image', `${sha256(written).slice(0, 16)} vs ${sha256(first!).slice(0, 16)}`);
  check(reported.elapsedMs < 1000, 'a whole second process rendered in well under a second', `${reported.elapsedMs}ms in-process, ${secondWall}ms including node`);

  checks.section('one connection, several requests at once');
  ...
  const batch = await Promise.all(Array.from({length: 10}, () => client.screenshot(request).then((result) => result.image!)));
  check(batch.length === 10 && batch.every((png) => sha256(png) === sha256(first!)), 'ten concurrent requests all come back, all identical', `${Date.now() - batchStarted}ms for ten`);

  checks.section('a failed request is not a dead daemon');
  const rejected = await client.screenshot({file: corpus, selector: '#nothing-here'}).then(() => null, (e: unknown) => e as Error);
  check(rejected !== null && /no element matches/.test(rejected.message), "a selector that matches nothing rejects with the worker's message", rejected ? rejected.message.slice(0, 60) : 'no error');
  ...
  checks.section('it also goes away on its own');
...
const cli = cac('pnpm verify:daemon');
cli.command('<exe>', 'exercise the resident daemon against a real engine')
