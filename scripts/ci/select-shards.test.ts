...
import {changesNeedCompilation, shardCount} from './select-shards.ts';

test('warm documentation/version changes avoid duplicate runners; unknown inputs do not', () => {
  assert.equal(changesNeedCompilation(['apps/typescript/package.json', 'apps/docs/release.md', 'apps/typescript/README.md']), false);
