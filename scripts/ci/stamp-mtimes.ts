...
//   * runtime feature definitions and Blink generator helpers are content
//     tracked against the restored cache, so switching branches cannot leave
//     newer generated headers describing different source bytes.
...
function main(workspaceArg: string, solutionName: string, outDir: string): number {
  ...
  const runtimeFeatures = createRuntimeFeatureMtimes(path.join(solution, outDir, 'Shot'), head);
