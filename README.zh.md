...
Shotium 在官方持续集成环境中，跨 6 大平台架构与主流无头浏览器方案进行严格的矩阵对标测试（基于 Linux x64 标准云端环境实测，采用各自官方默认基准配置）：

| 引擎方案 | 冷启动首张 (p50) | 预热截图 (p50) | 吞吐量 (单并发) | 下载体积 (压缩后) | 安装体积 (压缩前) |
|:---|---:|---:|---:|---:|---:|
| **Shotium** | **56 ms** | **13.7 ms** | **24.5 张/秒** | **~11 MB** | **~32 MB** |
| Puppeteer (headless-shell) | 567 ms (10.1×) | 133.1 ms (9.7×) | 5.5 张/秒 | ~130 MB | ~380 MB |
| Playwright (headless-shell) | 698 ms (12.5×) | 128.8 ms (9.4×) | 5.8 张/秒 | ~130 MB | ~390 MB |
| Puppeteer (Chrome 完整浏览器) | 813 ms (14.5×) | 183.1 ms (13.4×) | 4.0 张/秒 | ~170 MB | ~450 MB |
| Playwright (Chrome 完整浏览器) | 934 ms (16.7×) | 153.0 ms (11.2×) | 5.0 张/秒 | ~170 MB | ~480 MB |
