...
interface StartOptions {
  /**
   * HTTP 磁盘缓存目录。默认是平台缓存目录下按项目区分的一个目录；
   * 传 null 关闭缓存。
   */
...
**0.3 起缓存默认开启**，这是相对 0.2 的反转。理由是量出来的：不开缓存时，每次截一个 `https:` URL 都要付 DNS、TLS 和一次往返，对一个小页面来说这是整个调用耗时的大头，也是调用方完全没预期要花的时间。至于「短命进程会留下一个目录」这个顾虑，答案是把目录按项目区分、加上容量上限、并放在平台自己的缓存目录里（系统和用户的清理工具都认得那个地方），而不是干脆不缓存。
...
缓存目录放在各平台放缓存的地方：Windows 是 `%LOCALAPPDATA%\shotium\Cache`，macOS 是 `~/Library/Caches/shotium`，其余平台是 `$XDG_CACHE_HOME/shotium`（通常就是 `~/.cache/shotium`）。不放系统临时目录 —— 那个目录的定义就是「不用活到明天」：`/tmp` 重启即清，十天没碰过的文件也会被清掉，而一个全部价值都在下一次运行的缓存不能住在那里。里面按项目分目录，目录名是项目根路径的哈希。
